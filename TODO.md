...
license/NOTICE 已 confirmed；当前最新 GitHub Release 是 `v0.1.0-alpha.6`，后续 tag release 仍必须经过
...
不代表当前已有 live browser capture；`launch --confirm` 增量晚于 `v0.1.0-alpha.6`，进入下载二进制需要下一次
...
- [x] Linux CLI 二进制发布路径从 `v0.1.0-alpha.2` 打通，当前最新 GitHub Release 为 `v0.1.0-alpha.6`：release workflow 已在 GitHub Actions 中完成 `package-linux`、checksum、manifest、attestation、publish eligibility 和 tag release asset 上传，GitHub Release 已包含 Linux CLI tarball、sha256、manifest 和 manifest sha256；未运行本地构建、测试或打包。
