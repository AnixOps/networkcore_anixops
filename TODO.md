...
- [ ] 在 `engine-native` 中补充从有效配置图生成首个 native runtime assembly plan 的源码合同，选择 loopback TCP listener 与 SOCKS outbound handler，并覆盖失败释放边界，继续不接入 `networkcore-linux start`。
