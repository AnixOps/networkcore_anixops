...
license/NOTICE 已 confirmed；当前最新 GitHub Release 是 `v0.1.0-alpha.7`，后续 tag release 仍必须经过
同 commit CI、checksum、manifest、attestation、release notes、rollback 和 publish eligibility gates。
MITM CLI command gate 已进入状态/诊断/证书计划/浏览器捕获计划、browser-capture local proxy endpoint verify 和 blocked report 部分激活：
`mitm-cli-command-gate-status=partial-active`；browser hijack 仍为 deferred，浏览器捕获 mutation 仍 blocked。
Linux MITM browser capture source contract 已激活，当前源码已有 `mitm browser-capture plan/launch-plan/launch/apply/rollback/verify`
manual launch-plan、显式授权 dedicated-profile launch、本地代理端点 verify、blocked report 命令面和 `browser_capture` 机器字段，但只记录手动启动模板、进程启动报告、授权、快照、local proxy endpoint probe、apply/rollback blocked report 和 CI 边界，
不代表当前已有 live browser capture；`launch --confirm` 已进入当前 Linux CLI artifact 边界，但仅启动 dedicated browser profile，
不会写系统代理、浏览器 policy、PAC、TUN、DNS、firewall 或 CA 状态。
注意区分 release 与 main 源码：`v0.1.0-alpha.7` 是当前最新用户可下载 Linux artifact；`verify --confirm` 本地代理端点探测已在 main 源码边界中，进入下载包需要后续新 tag release。
...
- [ ] 补齐用户可用 MITM 后续门禁：`MITM_CLI_COMMAND_GATE` 已有 `networkcore-linux mitm status/diagnostics/certificate-plan/browser-plan`、`networkcore-linux mitm browser-capture plan/launch-plan/launch/apply/rollback/verify`、`mitm_status` JSON 字段、`certificate_plan`、`browser_plan` 和 `browser_capture` 机器字段，但仍需扩展到真正可执行的用户操作；`MITM_CERTIFICATE_LIFECYCLE_GATE` 当前为 plan-only，后续必须实现 CA 生成、安装、信任检测、撤销和回滚；`MITM_BROWSER_CAPTURE_GATE` 当前为 plan-only/mutation-blocked，且已有 [Linux MITM Browser Capture Source Contract](docs/architecture/linux-mitm-browser-capture-source-contract.md) 固定 `BrowserCaptureProcessRunner`、`BrowserCaptureEndpointProbe`、`LinuxBrowserCaptureLaunchRequest`、`LinuxBrowserCaptureLaunchReport`、`LinuxBrowserCaptureVerifyRequest`、`BrowserCaptureAuthorization`、`BrowserCaptureRollbackSnapshot`、launch-plan、launch、apply/rollback/verify、授权和 snapshot 边界，后续必须实现显式授权的浏览器/系统代理配置、PAC 或其他捕获策略、浏览器真实流量验证和回滚；`MITM_HTTP_TLS_DATA_PLANE_GATE` 在 HTTP/TLS 数据面中应用 `mitm-policy` URL/header/body/script rewrite plan。当前 `mitm-policy` 和 Linux CLI 只做策略状态、证书计划、浏览器捕获计划、browser-capture manual launch-plan/dedicated-profile launch/local proxy endpoint verify/blocked report、deferred mutation 诊断和 browser hijack deferred 输出，不能直接拦截或改写真实流量。
...
- [x] 扩展 Linux MITM browser capture manual launch-plan、dedicated-profile launch 与 blocked report 命令面：`networkcore-linux mitm browser-capture plan/launch-plan/launch/apply/rollback/verify` 已输出 `browser_capture` 机器字段；`launch-plan` 输出绑定 `networkcore.adblock` 插件元数据和计划代理 URL 的手动 dedicated-profile 浏览器启动命令模板且不写系统状态；`launch --confirm` 通过 `BrowserCaptureProcessRunner` 启动带显式代理参数的 dedicated browser profile，并输出 `LinuxBrowserCaptureLaunchReport`、pid、profile、proxy、命令参数和插件元数据；`apply --confirm` 只记录 `BrowserCaptureAuthorization` 并返回 `cli.linux.mitm.browser_capture.apply.blocked`，`rollback --snapshot <path>` 只保留 `BrowserCaptureRollbackSnapshot` 路径并返回 rollback blocked；系统代理、浏览器 policy、PAC、TUN、DNS、firewall、CA 和 HTTP/TLS 数据面仍不执行 mutation。验证仍只通过 GitHub Actions。
- [x] 扩展 Linux MITM browser capture verify 本地代理端点探针：`networkcore-linux mitm browser-capture verify --confirm` 已通过 `BrowserCaptureEndpointProbe` 探测计划本地代理端点 `http://127.0.0.1:7890`，输出 `LinuxBrowserCaptureVerifyRequest`、`verify_report`、proxy URL、probe 类型、`networkcore.adblock` 插件元数据、`cli.linux.mitm.browser_capture.verify.proxy_reachable` 或 `cli.linux.mitm.browser_capture.verify.proxy_unreachable` 诊断；缺少 `--confirm` 时返回 authorization required 且不探测。该命令不证明浏览器真实流量捕获、HTTPS MITM 或 rewrite 应用，也不写系统代理、浏览器 policy、PAC、TUN、DNS、firewall 或 CA 状态。验证仍只通过 GitHub Actions。
...
- [x] Linux CLI 二进制发布路径从 `v0.1.0-alpha.2` 打通，当前最新 GitHub Release 为 `v0.1.0-alpha.7`：release workflow 已在 GitHub Actions 中完成 `package-linux`、checksum、manifest、attestation、publish eligibility 和 tag release asset 上传，GitHub Release 已包含 Linux CLI tarball、sha256、manifest 和 manifest sha256；未运行本地构建、测试或打包。
