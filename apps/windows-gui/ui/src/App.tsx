import {
  Activity, BadgeCheck, Cable, CheckCircle2, ChevronRight, CircleAlert, ClipboardList, CloudDownload,
...
function Home({ snapshot, busy, run }: { snapshot: RuntimeSnapshot | null; busy: string | null; run: (name: string, task: () => Promise<{ message: string }>) => Promise<void> }) {
  ...
  return <div className="page-grid home-grid">
    ...
    <section className="panel span-2"><div className="panel-header"><div><h3>Session details</h3><p>Only independently verified runtime state is shown as connected.</p></div><button className="icon-text" onClick={() => void run("restart", desktop.restart)} disabled={Boolean(busy)}><RefreshCw size={16} />Restart {snapshot?.runtimeMode === "desktop" ? "core" : "service"}</button></div>
...
function Subscriptions({ snapshot, busy, run }: { snapshot: RuntimeSnapshot | null; busy: string | null; run: (name: string, task: () => Promise<{ message: string }>) => Promise<void> }) { const [location, setLocation] = useState(snapshot?.subscription ?? ""); const [sources, setSources] = useState<SubscriptionSummary[]>([]); const refreshSources = () => void desktop.subscriptions().then(setSources); useEffect(() => { setLocation(snapshot?.subscription ?? ""); refreshSources(); }, [snapshot?.subscription]); const refreshDetail = snapshot?.subscriptionError ?? (snapshot?.subscriptionLastAttempt ? `${snapshot.subscriptionRefreshResult || "pending"}; attempt ${snapshot.subscriptionLastAttempt}; +${snapshot.subscriptionAddedNodeCount} -${snapshot.subscriptionRemovedNodeCount} ~${snapshot.subscriptionChangedNodeCount}${snapshot.subscriptionNextAttempt ? `; next ${snapshot.subscriptionNextAttempt}` : ""}${snapshot.subscriptionRefreshErrorCode ? `; ${snapshot.subscriptionRefreshErrorCode}` : ""}` : "Generated NodeCatalog profiles enable selection and delay testing."); return <div className="page-grid"><section className="panel span-2"><div className="panel-header"><div><h3>Subscriptions</h3><p>Import a local profile or an explicit HTTP(S) URL.</p></div><StatusPill label={snapshot?.subscription ? "Configured" : "No source"} tone={snapshot?.subscription ? "success" : "neutral"} /></div><div className="subscription-card"><CloudDownload size={26} /><div><strong>{snapshot?.subscription ?? "No subscription configured"}</strong><p>{refreshDetail}</p></div><ChevronRight size={20} /></div><div className="toolbar subscription-toolbar"><input aria-label="Profile path or subscription URL" placeholder="Profile path or HTTPS subscription URL" value={location} onChange={(event) => setLocation(event.target.value)} /><button className="primary" disabled={Boolean(busy) || !location.trim()} onClick={() => void run("subscription", async () => { const result = await desktop.importSubscription(location); refreshSources(); return result; })}>Import profile</button><button className="secondary" disabled={Boolean(busy) || !snapshot?.subscription?.startsWith("http")} onClick={() => void run("subscription", desktop.updateSubscription)}>Update saved URL</button><button className="secondary" disabled={Boolean(busy) || !snapshot?.subscription} onClick={() => void run("selector", desktop.checkProfileRuntime)}>Check selector</button></div><div className="node-table"><div className="node-head"><span>Saved source</span><span>Updated</span><span>State</span><span /></div>{sources.map((source) => <div className="node-row" key={source.id}><strong>{source.location}</strong><span>{source.lastSuccessfulUpdate ?? source.lastAttempt ?? "Never"}</span><StatusPill label={source.selected ? "Active" : (source.result === "failed" ? "Error" : source.result || "Saved")} tone={source.selected ? "success" : (source.result === "failed" ? "danger" : "neutral")} /><div className="row-actions"><button className="secondary compact" disabled={Boolean(busy) || source.selected} onClick={() => void run("subscription", () => desktop.selectSubscription(source.id))}>Select</button><IconButton label={`Remove ${source.location}`} disabled={Boolean(busy)} onClick={() => void run("subscription", async () => { const result = await desktop.removeSubscription(source.id); refreshSources(); return result; })}><CircleAlert size={16} /></IconButton></div></div>)}</div></section></div>; }
...
function Preferences({ snapshot, busy, run }: { snapshot: RuntimeSnapshot | null; busy: string | null; run: (name: string, task: () => Promise<{ message: string }>) => Promise<void> }) {
  ...
  const [mieruUrl, setMieruUrl] = useState("");
  const [mieruPath, setMieruPath] = useState("");
  const [mieruSha256, setMieruSha256] = useState("");
  ...
  return <div className="page-grid"><section className="panel"><div className="panel-header"><div><h3>Runtime mode</h3><p>Choose how the proxy core is owned for daily connections.</p></div></div><div className="segmented-control" role="group" aria-label="Runtime mode"><button className={values.runtimeMode === "desktop" ? "selected" : ""} onClick={() => setValues({ ...values, runtimeMode: "desktop" })}>Desktop</button><button className={values.runtimeMode === "service" ? "selected" : ""} onClick={() => setValues({ ...values, runtimeMode: "service" })}>Windows service</button></div><p className="field-hint">Desktop runs sing-box only for this user. Service mode is required for tunnel, Mieru, and HTTPS MITM.</p><div className="subsection"><h4>Daily startup</h4>{(["startAfterLogin", "autoConnect", "autoRecoverCore", "autoSubscriptionRefresh", "autoSelectFastestNode"] as const).map((key) => <ToggleRow key={key} label={{ startAfterLogin: "Start after sign-in", autoConnect: "Connect after startup", autoRecoverCore: "Recover one GUI-started core failure", autoSubscriptionRefresh: "Refresh saved subscription hourly", autoSelectFastestNode: "Select fastest node every 30 minutes" }[key]} checked={values[key]} onChange={() => toggle(key)} />)}</div><button className="primary" disabled={Boolean(busy)} onClick={() => void run("preferences", () => desktop.savePreferences(values))}>Save preferences</button></section><section className="panel"><div className="panel-header"><div><h3>Managed configuration</h3><p>Install and verify operator-supplied public cores before configuring a profile.</p></div></div><div className="stack-actions"><button className="primary" disabled={Boolean(busy)} onClick={() => void run("core", desktop.installCore)}><Cpu size={17} />Install sing-box</button><button className="secondary" disabled={Boolean(busy)} onClick={() => void run("validate", desktop.validate)}><BadgeCheck size={17} />Validate configuration</button></div><div className="subsection"><h4>Mieru external core</h4><input aria-label="Mieru official release URL" placeholder="Official Mieru GitHub release asset URL" value={mieruUrl} onChange={(event) => setMieruUrl(event.target.value)} /><input aria-label="Mieru executable path" placeholder="Mieru executable destination" value={mieruPath} onChange={(event) => setMieruPath(event.target.value)} /><input aria-label="Mieru SHA-256 digest" placeholder="Mieru SHA-256 digest" value={mieruSha256} onChange={(event) => setMieruSha256(event.target.value)} /><div className="stack-actions"><button className="primary" disabled={Boolean(busy) || !mieruUrl || !mieruPath || !mieruSha256} onClick={() => void run("mieru install", () => desktop.installMieru(mieruUrl, mieruPath, mieruSha256))}><CloudDownload size={17} />Install Mieru</button><button className="secondary" disabled={Boolean(busy) || !mieruPath || !mieruSha256} onClick={() => void run("mieru verify", () => desktop.verifyMieru(mieruPath, mieruSha256))}><CheckCircle2 size={17} />Check Mieru</button></div></div></section></div>;
}
...
function Advanced({ snapshot, busy, run }: { snapshot: RuntimeSnapshot | null; busy: string | null; run: (name: string, task: () => Promise<{ message: string }>) => Promise<void> }) {
  ...
  const routingModes = [{ value: "bypass_china", label: "Bypass China" }, { value: "gfw_list", label: "GFW list" }, { value: "direct", label: "Direct" }, { value: "return_china", label: "Return China" }] as const;
  ...
  return <div className="page-grid">
    <section className="panel span-2">
      <div className="panel-header"><div><h3>Routing</h3><p>Rule assets are cached locally from curated public sources and applied on the next connection.</p></div><StatusPill label={routingModes.find((mode) => mode.value === snapshot?.routingMode)?.label ?? "Bypass China"} tone="neutral" /></div>
      <div className="segmented-control routing-mode" role="group" aria-label="Routing mode">{routingModes.map((mode) => <button key={mode.value} className={snapshot?.routingMode === mode.value ? "selected" : ""} disabled={Boolean(busy)} onClick={() => void run("routing", () => desktop.setRoutingMode(mode.value))}>{mode.label}</button>)}</div>
      <div className="stack-actions"><button className="secondary" disabled={Boolean(busy)} onClick={() => void run("routing rules", desktop.refreshRoutingRules)}>Refresh rule assets</button></div>
    </section>
    ...
    <section className="panel span-2 danger-panel">
      ...
      <ToggleRow label="Bilibili web promotional request blocking (trial)" checked={Boolean(snapshot?.bilibiliWebAdBlockEnabled)} onChange={() => void run("Bilibili web trial", async () => {
        if (!snapshot) throw new Error("Runtime status is not available.");
        await desktop.savePreferences({ startAfterLogin: snapshot.startAfterLogin, autoConnect: snapshot.autoConnect, autoRecoverCore: snapshot.autoRecoverCore, autoSubscriptionRefresh: snapshot.autoSubscriptionRefresh, autoSelectFastestNode: snapshot.autoSelectFastestNode, bilibiliWebAdBlockEnabled: !snapshot.bilibiliWebAdBlockEnabled, darkTheme: snapshot.darkTheme, runtimeMode: snapshot.runtimeMode });
        return { message: "Bilibili web trial selection saved. Enable HTTPS MITM again to apply it." };
      })} />
      <div className="stack-actions">
