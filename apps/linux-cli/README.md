...
`v0.1.2-alpha.3` release increment: `start --enable-https-mitm --mitm-ca-cert <ca.pem> --mitm-ca-key <ca.key> --confirm` opts into the controlled HTTP CONNECT TLS path, which issues an authority-bound leaf, decrypts up to four bounded HTTP/1.1 request/response exchanges on one TLS session, forwards upstream with web-PKI validation, and applies the built-in plugin hook in both phases. The session closes on HTTP close semantics, terminal rewrite, parse failure, or the exchange budget. `start --enable-script-runtime --script-runner <local-runner> --script-map <script-url>=<local-file> [--script-map ...] [--script-store <path>] [--node-binary <path>] --confirm` attaches the local Node script executor to that hook. The runner and each mapped asset must already be local files; each mapped asset is SHA-256 pinned at executor creation and rechecked before dispatch, with missing, symbolic-link, unreadable, or changed assets kept fail-open. NetworkCore never downloads script URLs, and scripts remain trusted local code rather than a sandbox. These flags do not install CA trust or alter system/browser proxy state. The `mitm http-rewrite` preview report deliberately remains preview-only and does not run scripts; the opt-in `start` runtime is independently covered by the same-commit GitHub Actions gate.
