...
`BrowserCaptureRollbackSnapshot`, explicit launch/apply/rollback/verify commands,
and rollback-safe snapshots before any browser/system proxy state can be written.
The current command surface can launch a dedicated browser process after
`--confirm`, but it does not mutate system proxy, browser policy, PAC, TUN, DNS,
firewall, CA, or HTTP/TLS data-plane state.
