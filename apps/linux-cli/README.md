...
and `mitm browser-capture apply --confirm --pac-file <path> --snapshot <path>`
/ `rollback --snapshot <path>` PAC artifact apply/rollback. The traffic-proof
command verifies that an operator-provided proof log contains a browser proof
token and emits `traffic_proof_report`; the PAC artifact path writes only the
operator-provided NetworkCore PAC file plus rollback snapshot and never installs
system PAC or browser policy. These source-only increments are not included in
`v0.1.0-alpha.8` and need a later tag release before users can download them.
...
`--confirm`, and it can write/rollback a caller-selected NetworkCore PAC
artifact with `--pac-file` plus `--snapshot`; it does not mutate system proxy,
browser policy, system PAC, TUN, DNS, firewall, CA, or HTTP/TLS data-plane
state.
