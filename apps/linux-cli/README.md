...
tag, including `mitm browser-capture verify --confirm`, require a later tag
...
`BrowserCaptureEndpointProbe`, `BrowserCaptureRollbackSnapshot`, explicit launch/apply/rollback/verify commands,
and rollback-safe snapshots before any browser/system proxy state can be written.
The current command surface can launch a dedicated browser process and verify
the planned local proxy endpoint after `--confirm`, but it does not mutate
