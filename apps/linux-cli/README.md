...
tag, including `mitm browser-capture verify --confirm` and `mitm browser-capture session-plan`, require a later tag
...
`LinuxBrowserCaptureSessionPlanReport`, `BrowserCaptureRollbackSnapshot`, explicit session-plan/launch/apply/rollback/verify commands,
...
session, launch a dedicated browser process, and verify the planned local proxy
endpoint after `--confirm`, but it does not mutate
