...
`mitm browser-capture apply --confirm --pac-file <path> [--policy-file <path>] --snapshot <path>`
/ `rollback --snapshot <path>` PAC/browser policy artifact apply/rollback, plus
...
policy artifact, and rollback snapshot and never installs system PAC or browser
policy; the native MITM hook only writes SOCKS5 CONNECT
