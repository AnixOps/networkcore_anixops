...
use networkcore_linux::{
    ...
    handle_install_service_plan, handle_install_sing_box,
    ...
    UnavailableForegroundLifecycleHost, UnavailableProxyEngineService, CLI_ARGUMENT_VALUE_MISSING_CODE,
    CLI_CONFIG_EMPTY_CODE, CLI_CONFIG_PATH_MISSING_CODE,
...
#[cfg(unix)]
use networkcore_linux::{
    handle_managed_control_reload, handle_managed_control_rollback, handle_managed_control_status,
    handle_managed_control_stop, start_managed_control_socket_with_interrupter,
    ManagedControlInterrupter, ManagedControlRequest, CLI_ARGUMENT_UNKNOWN_CODE,
...
#[test]
fn managed_foreground_session_event_cli_initializes_non_overwriting_record() {
    let root = std::env::temp_dir().join(format!(
        "networkcore-managed-foreground-event-cli-init-contract-{}",
        std::process::id()
    ));
    let _ = std::fs::remove_dir_all(&root);
    std::fs::create_dir_all(&root).expect("managed event CLI init directory should be created");
    let event_path = root.join("session-event.json");
    let event_path_text = event_path
        .to_str()
    ...
    .expect("managed-event init command should parse");
    assert!(matches!(
        &command,
    ...
    assert!(response.ok);
    assert_eq!(response.command, "managed-event init");
    let report = response
        .managed_foreground_event_write
        .as_ref()
        .expect("managed event init response should include the write report");
    assert_eq!(report.session_id, "session-event-cli-init");
    assert_eq!(report.engine_id, "native");
    assert_eq!(report.event_id, "event-cli-init-1");
    assert_eq!(report.event_kind, "status_transition");
    assert_eq!(report.state, "running");
    assert_eq!(report.recorded_at, "2026-07-10T00:00:00Z");
    assert!(report.record_written);
    ...
    let record: serde_json::Value = serde_json::from_str(
        &std::fs::read_to_string(&event_path)
            .expect("managed event CLI init record should be readable"),
    )
    .expect("managed event CLI init record should be valid JSON");
    assert_eq!(record["schema_version"], 1);
    assert_eq!(record["session_id"], "session-event-cli-init");
    assert_eq!(record["engine_id"], "native");
    assert_eq!(record["event_id"], "event-cli-init-1");
    assert_eq!(record["event_kind"], "status_transition");
    assert_eq!(record["state"], "running");
    assert_eq!(record["recorded_at"], "2026-07-10T00:00:00Z");
    let text = render_response(&response, OutputFormat::Text);
    assert!(text.contains("managed foreground event record written: true"));
    assert!(text.contains("managed foreground liveness verified: false"));
    let json: serde_json::Value =
        serde_json::from_str(&render_response(&response, OutputFormat::Json))
            .expect("managed event init response should render JSON");
    assert_eq!(
        json["managed_foreground_event_write"]["event_id"],
        "event-cli-init-1"
    );
    assert_eq!(
        json["managed_foreground_event_write"]["record_written"].as_bool(),
        Some(true)
    );
    assert_eq!(
        json["managed_foreground_event_write"]["liveness_verified"].as_bool(),
        Some(false)
    );
    ...
    let duplicate = handle_entrypoint(
        ...
        .expect("duplicate managed-event init command should parse"),
        &platform,
    );
    assert!(!duplicate.ok);
...
fn managed_foreground_session_status_cli_initializes_non_overwriting_record() {
    ...
    std::fs::remove_dir_all(&root).expect("managed status CLI init directory should be removed");
}
...
fn subscription_catalog_update_persists_snapshot_and_redacts_location() {
    ...
    let _ = std::fs::remove_dir_all(&root);
    std::fs::create_dir_all(&root).expect("catalog update test directory should be created");
    let catalog_path = root.join("catalog.json");
    let snapshot_path = root.join("catalog.snapshot.json");
    let catalog_json = r#"{
  "schema_version": 1,
  "sources": [
    {"id": "work", "location": "inline:old-secret-subscription-payload"},
    {"id": "remote", "location": "https://example.test/sub?token=old-secret-token"}
  ]
}"#;
    std::fs::write(&catalog_path, catalog_json).expect("catalog should be written");

    let report = CommandSubscriptionCatalogStore::new()
        .update_source(&SubscriptionCatalogUpdateRequest {
            catalog_path: catalog_path.display().to_string(),
            snapshot_path: snapshot_path.display().to_string(),
            source_id: " work ".to_string(),
            location: " https://updated.example/sub?token=new-secret-token ".to_string(),
        })
        .expect("catalog update should persist the new location");
    ...
    assert!(report.location_redacted);
    let debug_report = format!("{report:?}");
    assert!(!debug_report.contains("old-secret-subscription-payload"));
    assert!(!debug_report.contains("new-secret-token"));
    ...
    assert_eq!(catalog["sources"][0]["id"], "work");
    assert_eq!(
        catalog["sources"][0]["location"],
        "https://updated.example/sub?token=new-secret-token"
    );
    ...
    let missing = CommandSubscriptionCatalogStore::new()
        .update_source(&SubscriptionCatalogUpdateRequest {
            ...
            source_id: "missing".to_string(),
            location: "inline:unused".to_string(),
        })
        .expect_err("missing source id should be rejected");
    assert_eq!(
        missing.code,
...
fn os_signal_interruption_source_maps_unix_signals_to_stable_diagnostics() {
    ...
    let managed_rollback =
        OsSignalForegroundLifecycleInterruptionSource::interruption_for_managed_control_rollback();
    ...
    assert_eq!(managed_rollback.reason, "managed-control-rollback");
    ...
    assert_eq!(observed_managed_rollback.reason, "managed-control-rollback");
    ...
    assert_diagnostic(
        &managed_rollback.diagnostics,
        CLI_START_MANAGED_CONTROL_ROLLBACK_REQUESTED_CODE,
    );
    assert_diagnostic(
        &observed_managed_rollback.diagnostics,
        CLI_START_MANAGED_CONTROL_ROLLBACK_REQUESTED_CODE,
    );
...
fn managed_control_socket_accepts_confirmed_stop_and_cleans_up() {
    ...
    assert!(matches!(
        status,
        LinuxCliCommand::ManagedControlStatus { .. }
    ));
    ...
    let unavailable_status =
        handle_managed_control_status(socket_path.to_str().expect("socket path should be UTF-8"));
