...
当前 P4 状态：Linux CLI artifact 已通过 tag release workflow 发布到 GitHub Release，Windows managed-client 的当前 source release 目标为 `v0.2.0-alpha.19`，最新 stable 为 `v0.1.0`；tag workflow 将生成 Linux tarball、sha256、manifest、manifest sha256 以及 Windows MSI 四件套和 portable ZIP 四件套。MSI install-time service start asynchronous；服务先完成 SCM `Running` handoff，GUI/CLI Start 只返回即时 SCM 状态，CI 会用无效配置验证启动请求不被 runtime 卡住。GUI `Start`/`Restart` 不再预先应用 managed system proxy；服务保存唯一 runtime proxy snapshot 并负责 apply/rollback。SCM `Running` 后服务会轮询其拥有的 sing-box process；异常退出持久化 failed transition/exit detail、停止 runtime resources、回滚 proxy snapshot 并停止服务。Windows GUI 现可将 native sing-box JSON 原样导入 service-owned config、运行非变更 `check -c` 预检、生成本地 diagnostics report，并直接打开 core log；`Profile / URL` 可手动导入本地文件或 HTTP(S) 订阅地址，`Load nodes` 显式显示 NodeCatalog 节点并选择确定性出口，保存成功后 `Update URL` 可显式更新该单个 URL，失败不覆盖当前 config，且不建立后台更新或订阅目录；Hysteria2/TUIC share-link 与 native outbound import 可生成 direct sing-box QUIC config；Trojan/VLESS/VMess share-link 与 catalog import 还会保留 TLS/REALITY/uTLS/Vision/VMess security/alter-id 及 WebSocket/gRPC/HTTP/HTTPUpgrade/V2Ray QUIC subset，均不进入 HTTP/3 MITM；当原生配置明确提供 `type: mixed`、`tag: mixed-in` inbound 时，GUI MITM 会保存原始 JSON 快照、只转为 `7891` upstream listener，并在关闭时恢复。历史 `v0.1.1-alpha.2` Windows manual-extract CLI zip 仅作为审计记录保留；所有已发布和规划 alpha/rc/stable 切片的能力边界见 [Alpha Release Feature Matrix](docs/alpha-release-feature-matrix.md)。Linux artifact release-state consistency marker 为 `linux-artifact-release-state=confirmed-release-path`，license/NOTICE 已 confirmed，但后续 tag release 仍必须通过同 commit CI、checksum、manifest、attestation、release notes、rollback 和 publish eligibility gates。

当前 Windows main source slice 为 `v0.2.0-alpha.19`：在既有 managed client、portable ZIP、Hysteria2/TUIC direct QUIC rendering、native JSON pass-through 和 V2Ray compatibility subset 之上，服务在运行时配置前完成 SCM handoff，GUI/CLI Start 不再同步等待 runtime ready，也不在 SCM 请求前重复写 managed system proxy；服务 runtime state 保存唯一 proxy snapshot，并在失败或停止时完成对应 rollback。服务还轮询其拥有的 sing-box child；异常退出时记录 `failed`、exit code 与 failure detail，执行托管资源清理和 proxy rollback 后停止 SCM，而非保留指向失效 loopback listener 的系统代理。GUI 可先运行同一 `sing-box check -c` 预检、失败时生成状态与本地日志 tail 的 diagnostics report，并显式加载 local/HTTP(S) source 的 NodeCatalog 节点后选择出口，再手动更新保存 URL。generated profile 的 loopback Clash API 已支持 `Check core`、一次性 `Test delay` 和显式 `Switch active`；自动 latency/`urltest`、后台订阅刷新、订阅目录/组、route/rule 拉取、自动服务重启、XHTTP/ECH/multiplex 推断、HTTP/2/HTTP/3 MITM 和 JavaScript dispatch 仍未启用。
