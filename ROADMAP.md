...
`v0.1.2-alpha.2` 的 `read_status`/`write_status`/`transition_status` source-only 切片已显式读取、初始非覆盖写入或受 expected state 保护地迁移 schema version 1 managed foreground session record；迁移保留原始 record snapshot，仅允许 `starting -> running/failed` 与 `running -> stopped/failed`，并固定 `liveness_verified=false`，不检查 live process、不接入 CLI 或 runtime control；
