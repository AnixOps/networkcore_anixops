...
`v0.1.2-alpha.2` 的 `read_status`/`write_status` source-only 切片已显式读取或非覆盖写入 schema version 1 managed foreground session record，返回 recorded state 并固定 `liveness_verified=false`，不检查 live process、不接入 CLI 或 runtime control；
