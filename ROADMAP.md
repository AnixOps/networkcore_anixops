...
当前 P4 状态：Linux CLI artifact 已通过 tag release workflow 发布到 GitHub Release，最新已发布版本为 `v0.1.0-alpha.12`，并包含 tarball、sha256、manifest 和 manifest sha256；所有已发布和规划 alpha 切片的能力边界见 [Alpha Release Feature Matrix](docs/alpha-release-feature-matrix.md)。Linux artifact release-state consistency marker 为 `linux-artifact-release-state=confirmed-release-path`，license/NOTICE 已 confirmed，但后续 tag release 仍必须通过同 commit CI、checksum、manifest、attestation、release notes、rollback 和 publish eligibility gates。Linux 仍是手动解压和 foreground 运行模型，不安装 daemon/service，不修改 TUN/DNS/firewall/certificate trust store。iOS 仍只允许 `apps/ios/README.md` source tree governance placeholder 和 upload blocked gates，不包含 Swift/Xcode/Network Extension target、签名、TestFlight/App Store upload 或 iOS release asset。完整用户可用 live MITM 尚未启用；`MITM_CLI_COMMAND_GATE` 当前做到 status/diagnostics/certificate-plan/browser-plan partial-active，marker 为 `mitm-cli-command-gate-status=partial-active`，当前 artifact 包含 `mitm certificate apply/rollback` certificate artifact lifecycle、`mitm http-rewrite plan/preview` caller-provided plain HTTP rewrite foundation、`http_rewrite` report，以及 `mitm browser-capture plan/launch-plan/session-plan/launch/apply/rollback/verify/traffic-proof` manual launch-plan、订阅到本地代理/浏览器/verify/traffic-proof 会话计划、显式授权 dedicated-profile launch、可选 `--target-url` dedicated profile 打开页面、本地代理端点/target route verify、proof-log-token/default proof binding、`--proxy-scheme socks5` native plugin proxy mode、PAC/browser policy artifact apply/rollback、native SOCKS5 CONNECT plugin reject 与 blocked report 命令面。`mitm certificate apply/rollback` 只写入或删除调用方显式指定的 NetworkCore certificate/private-key artifact 和 snapshot，不安装或信任 CA；`mitm http-rewrite preview` 只应用 plugin outcome 到 caller-provided plain HTTP input，不解密 TLS、不拦截真实浏览器或系统流量；`launch --confirm` 只启动 dedicated browser profile，不写系统或浏览器代理状态；`--proxy-scheme socks5` 只把 dedicated browser、PAC/policy artifact、verify 和 traffic-proof 显式计划绑定到 `socks5://127.0.0.1:7890`；`verify --confirm`、`verify --confirm --target-url <url>`、`session-plan`、`--target-url`、`traffic-proof` 和 PAC/browser policy artifact apply/rollback 分别只探测计划本地代理端点、验证目标 URL 代理通路、输出脱敏会话计划、把目标页面传给 dedicated browser command、检查 proof log token 或写入/删除 caller-selected NetworkCore PAC/policy artifact，不证明 HTTPS MITM 或 live rewrite 应用；native SOCKS5 CONNECT plugin reject 只把插件 `Reject` 应用为 CONNECT failure，不解密 HTTPS，也不应用 redirect/header/body/script rewrite 到真实 HTTP 请求/响应。`MITM_CERTIFICATE_LIFECYCLE_GATE` 当前为 artifact-lifecycle-active/trust-mutation-blocked，`MITM_HTTP_TLS_DATA_PLANE_GATE` 当前为 plain-http-rewrite-foundation-active/tls-decryption-blocked，`MITM_BROWSER_CAPTURE_GATE` 当前为 pac-policy-profile-prefs-active/system-mutation-blocked，browser hijack 仍 deferred。Linux MITM certificate lifecycle 已有 [source contract](docs/architecture/linux-mitm-certificate-lifecycle-source-contract.md) 固定 `certificate_lifecycle`、artifact apply/rollback、`--cert-file`、`--key-file`、snapshot 和 trust mutation blocked 边界；Linux MITM HTTP rewrite 已有 [source contract](docs/architecture/linux-mitm-http-rewrite-source-contract.md) 固定 `NativePlainHttpMessage`、`NativePlainHttpRewriteReport`、`LinuxMitmHttpRewriteReport`、`http_rewrite`、显式授权和 TLS/live traffic blocked 边界；Linux MITM browser capture 已有 source contract 固定 `LinuxBrowserCaptureSessionPlanRequest`、`LinuxBrowserCaptureSessionPlanReport`、`BrowserCaptureProcessRunner`、`BrowserCaptureEndpointProbe`、`BrowserCaptureTrafficProofProbe`、`BrowserCapturePacFileStore`、`LinuxBrowserCaptureLaunchRequest`、`LinuxBrowserCaptureLaunchReport`、`LinuxBrowserCaptureVerifyRequest`、`LinuxBrowserCaptureTrafficProofRequest`、`LinuxBrowserCaptureTrafficProofReport`、`LinuxBrowserCapturePacRequest`、`BrowserCaptureAuthorization`、`BrowserCaptureRollbackSnapshot`、`proxy_scheme`、launch-plan、session-plan、target URL、target route verify、traffic-proof、PAC/browser policy artifact apply/rollback、launch、verify 命令面、授权、快照和回滚边界，但真实浏览器/系统代理 mutation 仍未实现；后续完整 HTTPS rewrite 仍需后续新 tag release 才会进入用户可下载 artifact。
