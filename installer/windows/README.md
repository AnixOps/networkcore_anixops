...
The GUI Home `Connect` command waits off the UI thread for SCM, the
service-owned sing-box PID, and the configured loopback listener. For the
NodeCatalog configuration generated by NetworkCore it also requires the local
Clash selector API to respond. Only then does it apply the configured proxy for
the interactive user and atomically save that desktop snapshot. The snapshot is
restored before disconnect and after a later observed core/config failure. The
manual GUI `Restore network settings` action restores only a snapshot owned by
that GUI session; it cannot enable an unverified proxy endpoint.
...
stops runtime resources, restores a service-owned proxy snapshot when the
configuration requests it, and returns SCM to `Stopped` rather than leaving a
system proxy pointed at an unavailable listener.
...
`system_proxy_owner: "service"` retains the CLI/service behavior. GUI-imported
profiles write `"desktop"` instead, so the interactive GUI owns the exact
current-user proxy snapshot and rollback. `sing_box` is optional. When enabled, the service first runs
...
3. Click `Import profile`, install the service once if needed, then use Home
   `Connect`. After a successful URL import, click `Update URL` to explicitly
   refresh that saved URL.
