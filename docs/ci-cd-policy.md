...
- managed foreground session event source contract、schema version、explicit event record、allowed event kind、recorded state、recorded_at、read-only `networkcore-linux managed-event <event-record-path>`、`CommandManagedForegroundSessionEventStore::read_event` and contract tests、`liveness_verified=false`、以及 no default path/event write/list/scan/stream、no PID/port/socket liveness、no daemon/control socket、no runtime control boundary 检查
