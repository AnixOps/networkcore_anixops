...
- Linux MITM browser capture source contract、`LinuxBrowserCaptureManualLaunch`、`LinuxBrowserCaptureLaunchRequest`、`LinuxBrowserCaptureLaunchReport`、`BrowserCaptureProcessRunner`、`BrowserCaptureAuthorization`、`BrowserCaptureRollbackSnapshot`、manual dedicated-profile launch-plan、显式授权 dedicated-profile launch、snapshot、rollback、launch-plan ready diagnostics、launch authorization/started/failed diagnostics、apply/rollback/verify blocked diagnostics、`browser_capture` report 和 `MITM_BROWSER_CAPTURE_GATE` plan-only/mutation-blocked 检查
