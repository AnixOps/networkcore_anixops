...
iOS Network Extension 当前只允许先做 design 和 static governance 检查；出现 Swift、Xcode project、Network Extension target 或签名验证后，相关 `swift build`、`swift test`、`xcodebuild`、签名、TestFlight 或 App Store Connect 验证仍只能在 GitHub Actions 或 Apple 官方平台执行。
