...
- placeholder 阶段必须通过 release placeholder 和 release summary job 显式输出发布来源、policy、release-ci-gate、artifact contract、signing contract、rollback contract、Linux artifact readiness、Linux foreground stop/release contract、Linux artifact manifest contract、Linux artifact manifest output fields、Linux artifact license/NOTICE status、placeholder、artifact 状态和后续 artifact 门禁。
