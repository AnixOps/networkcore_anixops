...
- placeholder 阶段必须包含 `release-ci-gate` job，记录真实 artifact 加入前必须关联 `main` 上同 commit 的成功 CI 结果，并输出后续从 GitHub Actions 读取的 CI run/source 字段合同。
