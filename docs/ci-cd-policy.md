...
- placeholder 阶段必须包含 `linux-artifact-readiness` job，检查 Linux CLI 源码、platform adapter、native listener/node 配置设计、foreground stop/release 源码与合同测试、artifact manifest 合同设计、license/NOTICE confirmation source contract、安装/回滚设计和 license/NOTICE 人工确认记录，且不得生成 artifact。
