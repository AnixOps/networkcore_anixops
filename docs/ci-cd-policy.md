...
- persistent subscription catalog source contract、schema version、explicit path、snapshot、redacted add/list/remove/select/update report、`CommandSubscriptionCatalogStore::add_source`/`list_sources`/`remove_source`/`select_source`/`update_source` and contract tests、以及 no default/remote/file fetch boundary 检查
