...
- managed foreground session status source contract、schema version、explicit status record、recorded state、`record_written=true`、`liveness_verified=false`、`CommandManagedForegroundSessionStore::read_status`/`write_status` and contract tests、以及 no PID/port/socket liveness、no daemon/control socket、no CLI/runtime control boundary 检查
