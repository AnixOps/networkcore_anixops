...
- managed foreground session status source contract、schema version、explicit status/snapshot record、recorded state、expected-state transition、read-only `networkcore-linux managed-status <status-record-path>`、`record_written=true`、`snapshot_written=true`、`liveness_verified=false`、`CommandManagedForegroundSessionStore::read_status`/`write_status`/`transition_status` and contract tests、以及 no PID/port/socket liveness、no daemon/control socket、no CLI write/transition/runtime control boundary 检查
