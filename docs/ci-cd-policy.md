...
- managed foreground session event source contract、schema version、explicit event record、allowed event kind、recorded state、recorded_at、`CommandManagedForegroundSessionEventStore::read_event` and contract test、`liveness_verified=false`、以及 no default path/event write/list/scan/CLI/stream、no PID/port/socket liveness、no daemon/control socket、no runtime control boundary 检查
