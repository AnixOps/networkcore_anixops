...
- placeholder 阶段必须包含 `linux-artifact-readiness` job，检查 Linux CLI 源码、platform adapter、native listener/node 配置设计、foreground stop/release 源码与合同测试、artifact manifest 合同设计、安装/回滚设计和 license/NOTICE 人工确认记录，且不得生成 artifact。
- placeholder 阶段必须通过 release summary job 显式输出发布来源、policy、release-ci-gate、artifact contract、signing contract、rollback contract、Linux artifact readiness、Linux foreground stop/release contract、Linux artifact manifest contract、placeholder、artifact 状态和后续 artifact 门禁。
