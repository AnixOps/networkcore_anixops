...
- managed foreground session status source-only rollback contract、explicit status/snapshot path、expected-state、trimmed session/engine identity、raw snapshot restore、`snapshot_retained=true`、`ManagedForegroundSessionStatusRollbackRequest`/`ManagedForegroundSessionStatusRollbackReport`/`CommandManagedForegroundSessionStore::rollback_status` and contract test、no `managed-status` rollback CLI、no PID/port/socket liveness、no daemon/control socket、no runtime control boundary 检查
