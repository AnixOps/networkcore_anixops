...
- `mitm-policy` safe wrapper、`networkcore.adblock` 内置去广告插件、`control-domain` `HttpMitmEvent`/`HttpMitmOutcome` rich mutation plan、`MitmPluginService::handle_http_mitm_event`、`engine-native` `NativeHttpMitmPluginHook`、native SOCKS5 CONNECT plugin reject 应用、native SOCKS5 CONNECT browser proof 诊断、plain HTTP rewrite foundation、Linux CLI `native_proxy_engine_service_with_builtin_mitm_plugin`、Linux CLI `mitm_status`、`certificate_plan`、`certificate_lifecycle`、`http_rewrite`、`browser_plan` 与 `browser_capture` 机器字段、manual launch-plan output、redacted session-plan output、dedicated-profile launch report、dedicated profile target URL、target URL proxy route verify、proof-log-token traffic proof、`--proxy-scheme socks5` native plugin proxy mode、MITM certificate artifact apply/rollback、MITM HTTP rewrite plan/preview、PAC/browser policy/profile prefs apply/rollback、planned/deferred mutation 诊断、当前用户可用性边界、`MITM_CLI_COMMAND_GATE` partial-active、`MITM_CERTIFICATE_LIFECYCLE_GATE` artifact-lifecycle-active/trust-mutation-blocked、`MITM_BROWSER_CAPTURE_GATE` pac-policy-profile-prefs-active/system-mutation-blocked、`MITM_HTTP_TLS_DATA_PLANE_GATE` plain-http-rewrite-foundation-active/tls-decryption-blocked 和 Rust 合同测试检查
- Linux MITM certificate lifecycle source contract、`LinuxMitmCertificateLifecycleReport`、`LinuxMitmCertificateArtifactRequest`、`LinuxMitmCertificateArtifactApplyOutcome`、`LinuxMitmCertificateArtifactRollbackOutcome`、`LinuxMitmCertificateTrustPlan`、`MitmCertificateArtifactStore`、`CommandMitmCertificateArtifactStore`、`MitmCertificateAuthorization`、`MitmCertificateRollbackSnapshot`、`handle_entrypoint_with_certificate_lifecycle_io`、`certificate_lifecycle`、`--cert-file`、`--key-file`、certificate artifact snapshot、rollback、apply/rollback diagnostics 和 `MITM_CERTIFICATE_LIFECYCLE_GATE` artifact-lifecycle-active/trust-mutation-blocked 检查
- Linux MITM browser capture source contract、`LinuxBrowserCaptureManualLaunch`、`LinuxBrowserCaptureSessionPlanRequest`、`LinuxBrowserCaptureSessionPlanReport`、`LinuxBrowserCaptureLaunchRequest`、`LinuxBrowserCaptureLaunchReport`、`LinuxBrowserCaptureVerifyRequest`、`LinuxBrowserCaptureVerifyReport`、`LinuxBrowserCaptureTrafficProofRequest`、`LinuxBrowserCaptureTrafficProofReport`、`LinuxBrowserCapturePacRequest`、`LinuxBrowserCapturePacApplyOutcome`、`LinuxBrowserCapturePacRollbackOutcome`、`BrowserCaptureProcessRunner`、`BrowserCaptureEndpointProbe`、`BrowserCaptureTrafficProofProbe`、`BrowserCapturePacFileStore`、`BrowserCaptureAuthorization`、`BrowserCaptureRollbackSnapshot`、manual dedicated-profile launch-plan、redacted session-plan、可选 `--target-url` dedicated profile 打开页面、`proof_target_url`、`networkcore_proof_token`、`MITM_BROWSER_CAPTURE_DEFAULT_PROOF_LOG_PATH`、`MITM_BROWSER_CAPTURE_DEFAULT_PROXY_SCHEME`、`MITM_BROWSER_CAPTURE_NATIVE_PLUGIN_PROXY_SCHEME`、`proxy_scheme`、`traffic_proof_command`、显式授权 dedicated-profile launch、显式授权 local proxy endpoint verify、target route verify、proof-log-token traffic proof、traffic-proof default proof binding、CONNECT endpoint proof token binding、`engine.native.runtime.http_mitm_connect_browser_proof_observed`、`--proxy-scheme socks5` native plugin proxy mode、PAC artifact snapshot、browser policy artifact、Firefox dedicated profile prefs、`--policy-file`、`--profile-prefs-file`、`profile_prefs_file_path`、`profile_prefs_content`、`cli.linux.mitm.browser_capture.policy.write_failed`、`cli.linux.mitm.browser_capture.profile_prefs.write_failed`、rollback、launch-plan ready diagnostics、session-plan ready/config diagnostics、launch authorization/started/failed diagnostics、verify authorization/proxy reachable/target reachable/proxy unreachable diagnostics、traffic-proof authorization/observed/missing/log-unreadable/blocked diagnostics、PAC apply ready/config/write diagnostics、PAC snapshot read/write diagnostics、rollback ready/failed diagnostics、apply/rollback/verify blocked diagnostics、`browser_capture` report、`traffic_proof_report` 和 `MITM_BROWSER_CAPTURE_GATE` pac-policy-profile-prefs-active/system-mutation-blocked 检查
- Linux MITM HTTP rewrite source contract、`NativePlainHttpMessage`、`NativePlainHttpRewriteApplication`、`NativePlainHttpRewriteReport`、`plan_and_apply_plain_http_mitm`、`apply_http_mitm_outcome_to_plain_http_message`、plain HTTP header/body/terminal action application、script dispatch deferred、`LinuxMitmHttpRewriteReport`、`LinuxMitmHttpRewriteRequest`、`LinuxMitmHttpRewriteOutcomeReport`、`LinuxMitmHttpRewriteAuthorization`、`handle_mitm_http_rewrite_plan`、`handle_mitm_http_rewrite_preview`、`http_rewrite` report、`--url`、`--method`、`--phase`、`--status-code`、`--header`、`--body`、authorization/config/TLS blocked diagnostics、JSON/text output、`MITM_HTTP_TLS_DATA_PLANE_GATE` plain-http-rewrite-foundation-active/tls-decryption-blocked 检查
...
- Alpha Release Feature Matrix 检查，确保 README、ROADMAP、TODO、Release Strategy 和 alpha
  版本能力索引同步记录已发布 `v0.1.0-alpha.1` 到 `v0.1.0-alpha.12`，以及后续
  TLS/live HTTPS rewrite 的规划边界
