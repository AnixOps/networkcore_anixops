...
- `mitm-policy` safe wrapper、`networkcore.adblock` 内置去广告插件、`control-domain` `HttpMitmEvent`/`HttpMitmOutcome` rich mutation plan、`MitmPluginService::handle_http_mitm_event`、`engine-native` `NativeHttpMitmPluginHook`、native SOCKS5 CONNECT plugin reject 应用、native SOCKS5 CONNECT browser proof 诊断、Linux CLI `native_proxy_engine_service_with_builtin_mitm_plugin`、Linux CLI `mitm_status`、`certificate_plan`、`certificate_lifecycle`、`browser_plan` 与 `browser_capture` 机器字段、manual launch-plan output、redacted session-plan output、dedicated-profile launch report、dedicated profile target URL、target URL proxy route verify、proof-log-token traffic proof、`--proxy-scheme socks5` native plugin proxy mode、MITM certificate artifact apply/rollback、PAC/browser policy/profile prefs apply/rollback、planned/deferred mutation 诊断、当前用户可用性边界、`MITM_CLI_COMMAND_GATE` partial-active、`MITM_CERTIFICATE_LIFECYCLE_GATE` artifact-lifecycle-active/trust-mutation-blocked、`MITM_BROWSER_CAPTURE_GATE` pac-policy-profile-prefs-active/system-mutation-blocked、`MITM_HTTP_TLS_DATA_PLANE_GATE` blocked 和 Rust 合同测试检查
...
- Alpha Release Feature Matrix 检查，确保 README、ROADMAP、TODO、Release Strategy 和 alpha
  版本能力索引同步记录已发布 `v0.1.0-alpha.1` 到 `v0.1.0-alpha.11`，以及后续
  `v0.1.0-alpha.12` 的规划边界
