...
- placeholder 阶段必须包含 `ios-upload-readiness` job，检查 iOS upload workflow activation validation contract、iOS Swift/Xcode source tree activation preflight contract、iOS Package.swift source ownership activation preflight contract、`apps/ios/README.md` governance placeholder、`ios-upload-workflow` pending/blocked marker、protected environment/manual approval blocked 状态、App Store Connect API secret status not-read-blocked、source tree preflight、Package.swift ownership preflight、archive/export/upload/submission blocked 输出，且不得读取 secret、定义真实 iOS upload job 或生成 iOS release asset。
...
Swift/Xcode project、Network Extension target、签名、archive/export、upload 或 iOS release asset。出现 Swift、Xcode project、
