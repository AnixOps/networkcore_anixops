...
状态：发布准备中；本次 `v0.1.0-alpha.10` tag release 候选切片。
...
状态：规划中；已决定不包含 HTTPS rewrite。

预计窗口（非承诺）：2026-08-07 至 2026-08-14；scope 以 Linux CA artifact lifecycle
foundation 为核心，先固定证书生成、文件所有权、snapshot、rollback 和 trust-plan 边界。

目标方向：

- 新增 Linux MITM certificate lifecycle source contract，固定 CA artifact、私钥、snapshot、rollback、
  trust-plan、诊断 code 和 CI governance。
- 新增显式授权的 CA artifact 生成入口，写入 operator-provided cert/key 路径并拒绝覆盖未知文件。
- 新增 rollback 入口，通过 NetworkCore snapshot 删除或恢复 NetworkCore 管理的 CA artifact。
- 新增 trust-plan 或 trust-detection 计划输出，继续不执行 system trust store mutation。
