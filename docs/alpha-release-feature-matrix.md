...
- `v0.1.1-alpha.3`：订阅格式扩展。当前 source 增量接入 Trojan/VLESS/VMess URL parser gates、Clash YAML parser gate、sing-box JSON parser gate 和 Surge proxy line parser gate：
  `trojan://password@host:port?...#name`、`vless://uuid@host:port?...#name`、`vmess://base64(json)`、受支持的 Clash
  `proxies` 子集、sing-box JSON `outbounds` 子集以及 Surge `[Proxy]` line 子集只归一化到 `SubscriptionDocument`/`NodeCatalog`。本切片不包含节点选择、cross-platform run plan、Linux/Windows subscription run preview、远程 fetch、
