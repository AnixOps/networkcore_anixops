...
## `v0.2.0-alpha.1` source slice：Windows managed client MSI

状态：已进入 `main` 的实现阶段；只有同名 tag 的 GitHub Actions release
workflow 成功后，MSI 才成为用户可下载 artifact。

主要特性：

- `apps/windows-gui` 原生 Win32 GUI 激活 service 安装/启停/重启、托管配置导入、
  WinINet/WinHTTP proxy apply/restore、LocalMachine ROOT CA apply/remove 和 signed
  INF driver package install/remove。
- `apps/windows-service` 通过 SCM `AnixOpsNetworkCore` 宿主托管 EasyTier tunnel，
  以 schema-versioned ProgramData state 记录 apply/rollback，并在 stop/purge 中
  恢复或移除系统变更。
- `installer/windows/Package.wxs` 使用 WiX 4.0.6 生成 per-machine MSI；GitHub Actions
  生成 MSI、sha256、schema-version-2 manifest、manifest sha256 并执行 artifact attestation。

明确不包含：

- JavaScript script dispatch、Windows subscription runnable path、远程订阅拉取和默认路径扫描。
- 仓库不编译 kernel driver；driver lifecycle 只接收 Windows 验证签名的 operator-provided INF package。
