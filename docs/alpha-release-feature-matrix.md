...
状态：已发布；当前最新用户可下载 Linux artifact。
...
状态：发布准备中；本次 `v0.1.0-alpha.9` tag release 候选切片。

发布目标日期：2026-07-09；前提是同 commit CI、release workflow、checksum/manifest、
attestation、release notes/rollback 和 publish eligibility gates 全部通过。
...
预计窗口（非承诺）：2026-07-17 至 2026-07-24；scope 以显式授权 browser/system proxy 或
system PAC mutation、snapshot 和 rollback 为核心，若 CI 或 rollback 边界不达标则顺延。

目标方向：

- 在显式授权、snapshot 和 rollback 边界下推进 browser/system proxy 或 system PAC mutation。
- 把 `MITM_BROWSER_CAPTURE_GATE` 从 pac-policy-artifact-active/system-mutation-blocked 推向可回滚的真实捕获配置。
- 补齐更强的 live browser traffic proof 自动化。

明确不承诺：

- 如果 CA lifecycle 和 HTTP/TLS data plane 尚未完成，即使能修改代理/PAC，也仍不能称为 HTTPS MITM 或
  request/response rewrite 可用。

### `v0.1.0-alpha.11+`

状态：规划中；具体版本号可随 scope 调整。

预计窗口（非承诺）：2026-08-07 至 2026-08-21；scope 涉及 CA lifecycle 和 HTTP/TLS data
plane，若完整 HTTPS rewrite 风险过高，可能拆分到后续 alpha。

目标方向：

- 推进 `MITM_CERTIFICATE_LIFECYCLE_GATE`：CA 生成、安装、信任检测、撤销和回滚。
- 推进 `MITM_HTTP_TLS_DATA_PLANE_GATE`：把 `mitm-policy` `HttpMitmOutcome` redirect/header/body/script
  rewrite plan 应用到真实 HTTP/TLS 请求/响应。
- 在具备授权、证书、数据面、捕获策略和回滚后，才可评估完整 HTTPS MITM/browser hijack 版本边界。
