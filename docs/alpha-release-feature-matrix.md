...
- `v0.1.1-alpha.3`：订阅格式扩展。当前 source 增量接入 Trojan/VLESS/VMess URL parser gates：
  `trojan://password@host:port?...#name`、`vless://uuid@host:port?...#name` 和 `vmess://base64(json)` 只归一化到
  `SubscriptionDocument`/`NodeCatalog`。后续继续接入 Clash YAML、sing-box JSON 的 source contract 和 parser
