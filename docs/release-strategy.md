...
`v0.2.0-alpha.17`，当前 Windows source release 目标是 `v0.2.0-alpha.19`。该发布路径始终同时产出 Linux CLI 四件套、Windows managed-client MSI 四件套和 Windows portable ZIP 四件套。MSI 在安装期请求异步 service start，不等待 runtime ready；服务在配置 runtime 前完成 SCM handoff，GUI/CLI Start 立即返回观察到的状态，CI 对 MSI 执行 bounded install/uninstall、invalid-config nonblocking-start 和 core-exit proxy-rollback smoke。GUI `Start`/`Restart` 不再预先应用 managed system proxy；服务 runtime state 保存并回滚唯一 proxy snapshot，并监督其拥有的 sing-box process。异常核心退出会写入 failure detail、清理 runtime resources、恢复 proxy snapshot 并停止 SCM service。GUI 保留明确的 official sing-box core install、`check -c` 预检、本地 diagnostics、HTTP(S) profile import/update、NodeCatalog selector、`Check core`、手动 delay test 和明确 runtime switch；native JSON 保持 service-owned pass-through。MSI/portable ZIP 不捆绑或静默下载第三方 core，且不启用后台 refresh、自动 service restart、LAN controller/Web UI、`urltest`、XHTTP/ECH/multiplex 推断、HTTP/2/HTTP/3/QUIC MITM、streaming、多 request CONNECT 或 JavaScript dispatch。历史 `v0.1.1-alpha.2` Windows manual-extract CLI zip 仅保留为审计记录。Linux CLI
...
| Windows | `networkcore-windows` managed-client MSI and portable ZIP | `windows-latest` | `v0.2.0-alpha.19` keeps all translatable NodeCatalog nodes in a generated sing-box selector, explicit read-only controller health, confirmed runtime switching through the `127.0.0.1:9091` Clash API, and one operator-initiated HTTPS delay measurement. The service, rather than GUI Start/Restart, owns the managed proxy snapshot and supervises its owned core process: an unexpected exit records failure, rolls back runtime proxy state, and stops SCM. Native JSON stays pass-through; no LAN controller, Web UI, `urltest`, automatic latency selection, background refresh, persistent subscription catalog/group, or automatic restart is enabled. GUI managed-config/`sing-box check -c` preflight, local diagnostics, immediate SCM handoff, invalid-config/core-exit MSI smoke, portable ZIP, and operator-staged sing-box lifecycle remain active. |
