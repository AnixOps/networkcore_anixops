...
| Linux | 待定义 CLI 或 daemon 压缩包 | `ubuntu-latest` | [Linux artifact pre-release design](architecture/linux-artifact-pre-release-design.md)、[Linux platform adapter design](architecture/linux-platform-adapter.md) 与安装/卸载设计完成 |
...
- 真实平台产物进入 release workflow 前，先为目标平台补齐 adapter 设计文档；Linux 首个产物必须先满足 [Linux artifact pre-release design](architecture/linux-artifact-pre-release-design.md) 和 [Linux platform adapter design](architecture/linux-platform-adapter.md)。
