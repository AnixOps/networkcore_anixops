...
`v0.2.0-alpha.21` 是当前已发布 Windows managed-client prerelease；`v0.2.0-beta.1` 是当前 Beta release candidate，状态为 `pending_manual_acceptance`。Linux `v0.1.2-alpha.3` 仍是当前 Linux source slice。Windows MSI 与 portable ZIP 只在 GitHub Actions 中生成；正式 `v0.2.0-beta.1` tag 必须等 Windows/Linux 真实系统验收记录完成后再创建。
...
`v0.2.0-alpha.21`；当前 Beta candidate 是 `v0.2.0-beta.1`，状态为 `pending_manual_acceptance`。workflow_dispatch dry-run 可验证 Linux CLI 四件套、Windows managed-client MSI 四件套和 Windows portable ZIP 四件套，但只有真实 Windows/Linux 系统验收记录完成并且同名 tag workflow 全部成功后才成为可下载 release。该发布路径始终同时产出 Linux CLI 四件套、Windows managed-client MSI 四件套和 Windows portable ZIP 四件套。MSI 在安装期请求异步 service start，不等待 runtime ready；服务在配置 runtime 前完成 SCM handoff，CLI 继续只返回即时状态。GUI 日常 `Connect` 在后台验证 SCM、service-owned `check -c`、service-owned core PID、回环监听和生成 selector 的 active-outbound/default readback 后才应用当前 interactive-user proxy；GUI 仍保留明确的 official sing-box core install、本地 diagnostics、HTTP(S) profile import/update、NodeCatalog selector、`Check core`、手动 delay test 和明确 runtime switch。用户明确开启 hourly refresh 时，保存的单 HTTP(S) source 在后台执行一次有界刷新，持久展示脱敏 attempt/success/next/result、节点增删改计数和稳定错误码；失败保留现有 config、节点选择和运行配置，不重启服务或切换节点。native JSON 保持 service-owned pass-through。MSI/portable ZIP 不捆绑或静默下载第三方 core，且不启用自动 service restart、LAN controller/Web UI、`urltest`、XHTTP/ECH/multiplex 推断、HTTP/2/HTTP/3/QUIC MITM、streaming、多 request CONNECT 或 JavaScript dispatch。历史 `v0.1.1-alpha.2` Windows manual-extract CLI zip 仅保留为审计记录。Linux CLI
...
- release policy job 检查版本格式和触发来源一致性；手动 `workflow_dispatch` 验证必须从 `main` 分支发起，tag release 必须使用同名 tag 版本，版本可为稳定版、`alpha.N`、`beta.N` 或 `rc.N` 预发布版。
...
- `v0.2.0-beta.1` 是当前 Windows source candidate。它明确 desktop/service proxy
  ownership，在 GUI 应用当前用户代理前验证 SCM、`check -c`、live core PID、loopback listener
  和适用 generated selector 的 active-outbound/default readback；失败时回滚 GUI-owned proxy。它不将缺失 PID 显示为 `PID 0`，并在
...
| Windows | `networkcore-windows` managed-client MSI and portable ZIP | `windows-latest` | Current main is the `v0.2.0-beta.1` candidate. It requires independent SCM, `check -c`, listener, generated selector active-outbound/default readback, PID, and exact current-user proxy evidence before Connected or desktop proxy mutation; it retains explicit proxy ownership and truthful missing-PID handling in the shared-state tray, current-user Run startup, opt-in one-shot auto-connect/core recovery, exact GUI-owned proxy recovery, and MSI Run-entry cleanup baseline. It retains the generated NodeCatalog selector, loopback controller health/switch/delay, service-owned core supervision, native JSON pass-through, portable ZIP, and MSI path. A user-enabled single HTTP(S) subscription refresh records redacted attempt/success/next/result, node diff counts, and stable errors without restarting the core or switching nodes. No LAN controller, Web UI, `urltest`, automatic latency selection, subscription group, TUN/DNS interception, or script dispatch is enabled. |
...
- 版本号采用 `vMAJOR.MINOR.PATCH` tag 形式，alpha、beta 和 rc 预发布版本分别使用 `vMAJOR.MINOR.PATCH-alpha.N`、`vMAJOR.MINOR.PATCH-beta.N` 和 `vMAJOR.MINOR.PATCH-rc.N`；N 必须是无前导零的十进制正整数，为保持 Windows Installer 版本排序，分别只能为 `1-499`、`1-399` 和 `1-98`。当前 release policy gate 已按该格式和范围检查手动版本输入与 tag 名。
