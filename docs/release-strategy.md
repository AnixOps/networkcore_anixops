...
`v0.2.0-alpha.21` 是当前已发布 Windows managed-client prerelease；`v0.2.0-beta.3` 是当前 Beta release candidate，状态为 `pending_manual_acceptance`。Linux `v0.1.2-alpha.3` 仍是当前 Linux source slice。Windows MSI 与 portable ZIP 只在 GitHub Actions 中生成；维护者已授权 `v0.2.0-beta.3` tag 用于生成候选工件，但在 Windows/Linux 真实系统验收完成前不得描述为 production-ready。
...
- `v0.2.0-beta.3` 是当前 Windows source candidate。它明确 desktop/service proxy
