...
5. 每个上传产物必须生成 checksum；后续有 signing 或 attestation 能力时必须纳入同一 release run。
...
3. `package-*`：每个平台独立构建产物并输出 checksum。
4. `sign-*`：需要签名的平台在受控 runner 中读取 GitHub Secrets 或官方平台凭据。
5. `notarize-*`：macOS 产物完成 Apple notarization 后再进入发布资产。
6. `publish-github-release`：上传 release assets、checksums、release notes 和 provenance/attestation 信息。
7. `post-release-summary`：输出产物清单、验证链接、人工事项和回滚说明。
