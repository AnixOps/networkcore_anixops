...
- `v0.2.0-alpha.20` 是当前 Windows source release 目标。它保留相同 MSI/portable
  ...
  delay 操作在后台完成。详情见 [Windows GUI Daily Usability](architecture/windows-gui-daily-usability.md)。
...
| Windows | `networkcore-windows` managed-client MSI and portable ZIP | `windows-latest` | `v0.2.0-alpha.20` adds daily Home/Nodes/Subscriptions/Settings/Diagnostics/Advanced information architecture, SCM/core-PID/current-user-proxy runtime truth, and background GUI commands while retaining the generated NodeCatalog selector, explicit loopback controller health/switch/delay, service-owned core supervision, native JSON pass-through, portable ZIP, and MSI smoke path. No LAN controller, Web UI, `urltest`, automatic latency, background refresh, persistent subscription catalog/group, automatic restart, startup auto-connect, TUN/DNS interception, or script dispatch is enabled. |
