...
`v0.2.0-alpha.20` 是当前已发布 Windows managed-client prerelease；`v0.2.0-alpha.21` 是当前待发布的 Windows source candidate。Linux `v0.1.2-alpha.3` 仍是当前 Linux source slice。Windows MSI 与 portable ZIP 只在同名 tag 的 GitHub Actions 中生成。
...
- `v0.2.0-alpha.21` 是当前 Windows source candidate。它保留相同 MSI/portable
