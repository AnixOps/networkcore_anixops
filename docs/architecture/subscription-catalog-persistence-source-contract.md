...
本文定义 `v0.1.2-alpha.1` persistent subscription catalog 的前四个源码切片。
它约束显式本地 catalog 文件的 `add`、`list`、`remove` 和 `select` 操作，后续 `update`
必须在本合同基础上单独增加并通过 GitHub Actions 验证。
