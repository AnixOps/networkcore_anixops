...
- 本文档保持在 README、ROADMAP、Release Strategy、Release CI success source contract、Linux package
  manifest 设计、Linux package publish eligibility aggregate contract、Linux package publish/upload boundary
  contract、Linux CLI artifact 安装/回滚设计和 CI policy 中可发现。
...
- 下一步可以补充 Linux package artifact job preflight validation contract，明确 `package-linux`
  在 license/NOTICE 与 release CI gate 未解除前仍不定义，未来真实 job 的 checkout、toolchain、build、
  staging 前置顺序、失败条件和仍不上传 artifact 的边界。
