...
`release-ci-gate` API read 已激活；本文继续固定权限、API 读取字段和失败条件。Linux artifact
release path 也已激活，`package-linux`、attestation、publish eligibility 和 GitHub Release upload
必须继续由 GitHub Actions gates 约束。
...
当前 workflow 执行以上 API 读取并输出 active 状态；`package-linux` 已在 release workflow 中定义，
但必须依赖该 active CI gate。
...
- 历史 placeholder 不生成 artifact、不定义 `package-linux`、不定义 `publish-github-release`、不上传 workflow artifact、
