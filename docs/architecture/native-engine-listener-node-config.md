...
- `engine-native` 已补充 accepted TCP connection 的协议前置关闭诊断合同，在 proxy protocol 尚未实现时显式关闭 accepted connection，记录 pre-protocol close 计数和 `engine.native.runtime.connection_pre_protocol_closed` 诊断；该实现尚未接入 `NativeProxyEngineService::start`，也没有 proxy protocol parser 或 route/outbound 数据面。

因此，`engine-native` 现在必须继续拒绝启动。虽然配置服务已经能解析最小 listener/node/route 子集，adapter 已能校验 listener/node/route 图，且源码中已有 runtime handle、runtime assembly plan、loopback TCP listener resource、accept loop 受控关闭合同和协议前置关闭诊断，但在 proxy protocol parser、route/outbound 行为和真实运行句柄接线完成前，不得从 service `start()` 返回 `Running`，也不得接入 `networkcore-linux start`。
...
| `engine.native.runtime.connection_pre_protocol_closed` | Info | accepted TCP connection 在 proxy protocol 处理前被显式关闭 |
...
9. 已补充首个 accepted TCP connection 的协议前置关闭诊断合同，明确未实现 proxy protocol 时的连接处理边界，仍不接入 `networkcore-linux start`。
10. 下一步必须补充首个 SOCKS5 greeting 版本/认证方法读取诊断合同，继续不接入 route/outbound 或 `networkcore-linux start`。
11. 最后再评估 `networkcore-linux start` binary 接线和前台 lifecycle host handoff。
...
- adapter 已创建当前进程拥有的真实 listener/runtime handle，并具备 accept loop、受控关闭、proxy protocol parser、route/outbound 行为合同，而不仅是源码合同结构或 assembly plan。
...
- 只绑定端口、只生成 assembly plan、只启动 accept loop 或只做协议前置关闭诊断，但没有 proxy protocol parser、route/outbound 行为合同。
- 只能在测试替身中返回 `Running`。
...
- `engine-native` 在 listener/node 解析和图校验完成后，仍必须在 proxy protocol parser、route/outbound 行为和 service start 接线完成前继续保持 runtime unavailable 诊断。
...
- `engine-native` service 继续保持 runtime unavailable，直到 proxy protocol parser、route/outbound 行为和前台 lifecycle handoff 完成并通过 GitHub Actions 验证。
