...
50. Linux package artifact job preflight validation contract、Linux package artifact build command validation contract、Linux package artifact staging file validation contract 和 Linux package artifact archive creation validation contract 已定义；下一步补充 Linux package artifact checksum execution validation contract，仍不生成 artifact。
