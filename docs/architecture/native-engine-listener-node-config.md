...
17. 下一步必须补充 SOCKS5 outbound CONNECT request frame 生成诊断合同，继续不建立真实 outbound 连接、不接入 `networkcore-linux start`。
18. 最后再评估真实 outbound 连接、数据转发、`networkcore-linux start` binary 接线和前台 lifecycle host handoff。
...
- adapter 已创建当前进程拥有的真实 listener/runtime handle，并具备 accept loop、受控关闭、SOCKS5 认证方法响应、命令头读取、CONNECT 目标地址解析、route/outbound 行为选择、SOCKS outbound request/connection/data relay 和 failure/success response 写入合同，而不仅是源码合同结构或 assembly plan。
...
- 只绑定端口、只生成 assembly plan、只启动 accept loop、只做协议前置关闭诊断、只读取 SOCKS5 greeting、只选择/拒绝 SOCKS5 auth 方法、只写入认证方法响应、只读取 SOCKS5 命令头、只解析 CONNECT 目标地址、只选择 route/outbound 行为并报告未接入且写入 failure response，但没有 SOCKS outbound request/connection/data relay 合同。
- 只能在测试替身中返回 `Running`。
...
- `engine-native` 在 listener/node 解析和图校验完成后，仍必须在 SOCKS outbound request/connection/data relay 和 service start 接线完成前继续保持 runtime unavailable 诊断。
...
- `engine-native` service 继续保持 runtime unavailable，直到 SOCKS outbound request/connection/data relay 和前台 lifecycle handoff 完成并通过 GitHub Actions 验证。
