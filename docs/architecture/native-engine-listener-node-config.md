...
- `engine-native` 已补充 SOCKS5 命令头读取与 unsupported command 拒绝诊断合同，可在 no-auth 响应后读取 `[VER, CMD, RSV, ATYP]` 并记录 `engine.native.runtime.socks5_command_header_read`、`engine.native.runtime.socks5_command_header_invalid` 或 `engine.native.runtime.socks5_command_header_read_failed` 诊断，对非 CONNECT 命令记录 `engine.native.runtime.socks5_command_unsupported`；该实现尚未读取 CONNECT 目标地址，也没有 route/outbound 数据面。

因此，`engine-native` 现在必须继续拒绝启动。虽然配置服务已经能解析最小 listener/node/route 子集，adapter 已能校验 listener/node/route 图，且源码中已有 runtime handle、runtime assembly plan、loopback TCP listener resource、accept loop 受控关闭合同、协议前置关闭诊断、SOCKS5 greeting 读取诊断、auth 方法选择/拒绝诊断、认证方法响应写入诊断和 SOCKS5 命令头读取/unsupported command 拒绝诊断，但在 CONNECT 目标地址解析、route/outbound 行为和真实运行句柄接线完成前，不得从 service `start()` 返回 `Running`，也不得接入 `networkcore-linux start`。
...
14. 下一步必须补充 SOCKS5 CONNECT 目标地址读取与 route/outbound 未接入拒绝诊断合同，继续不接入 `networkcore-linux start`。
15. 最后再评估 `networkcore-linux start` binary 接线和前台 lifecycle host handoff。
...
- adapter 已创建当前进程拥有的真实 listener/runtime handle，并具备 accept loop、受控关闭、SOCKS5 认证方法响应、命令头读取、CONNECT 目标地址解析、route/outbound 行为合同，而不仅是源码合同结构或 assembly plan。
...
- 只绑定端口、只生成 assembly plan、只启动 accept loop、只做协议前置关闭诊断、只读取 SOCKS5 greeting、只选择/拒绝 SOCKS5 auth 方法、只写入认证方法响应或只读取 SOCKS5 命令头，但没有 CONNECT 目标地址解析、route/outbound 行为合同。
- 只能在测试替身中返回 `Running`。
...
- `engine-native` 在 listener/node 解析和图校验完成后，仍必须在 CONNECT 目标地址解析、route/outbound 行为和 service start 接线完成前继续保持 runtime unavailable 诊断。
...
- `engine-native` service 继续保持 runtime unavailable，直到 CONNECT 目标地址解析、route/outbound 行为和前台 lifecycle handoff 完成并通过 GitHub Actions 验证。
