...
- `config-core::CoreSubscriptionService`，当前只接受显式 `inline:` source，并把最小 subscription TOML `nodes`/`routes` 子集解析为 `SubscriptionDocument` 和 `NodeCatalog`；远程拉取、文件读取、认证、secret、重复 id 和 listener/node 图校验仍未进入。
