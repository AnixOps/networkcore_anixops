...
- Release CI success source contract、Linux package runner/toolchain/target contract、Linux
  ...
  Linux package artifact build command validation contract 和 Linux package artifact staging file validation
  contract 已定义；下一步可以补充 Linux package artifact archive creation validation contract，仍不生成 artifact。
