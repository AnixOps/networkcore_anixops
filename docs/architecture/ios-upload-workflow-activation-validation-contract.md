...
和 [iOS App Review Manual Confirmation Source Contract](ios-app-review-manual-confirmation-source-contract.md)。

当前状态：blocked-placeholder。仓库仍不包含 Swift source、`Package.swift`、Xcode project、Network Extension target、
...
- The repository still has no `PrivacyInfo.xcprivacy`, Swift/Xcode project, Network Extension target, ExportOptions.plist, `.ipa`,
...
`.github/workflows/release.yml` must output the activation fields in `ios-upload-readiness`, `release-placeholder` and
`release-summary`, and must make release summary depend on `ios-upload-readiness`.

## Acceptance Criteria

- README, ROADMAP, TODO, CHANGELOG, CI/CD policy, release strategy and this source contract chain are updated.
...
- No Swift source, `Package.swift`, Xcode project, Network Extension target, `PrivacyInfo.xcprivacy`, ExportOptions.plist,
