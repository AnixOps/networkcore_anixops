...
- 固定 source tree preflight contract 必须先输出 `apps/ios` README placeholder、`Package.swift`、Package.swift ownership、
  Xcode project、Network Extension target、`PrivacyInfo.xcprivacy`、entitlement/provisioning、upload enabled marker 和
  release/upload blocked 状态。
...
- The source tree preflight and Package.swift ownership contracts exist, and their `ios-source-tree-preflight` and
  `ios-package-swift-ownership` fields are emitted before upload activation fields.
...
- README, ROADMAP, TODO, CHANGELOG, CI/CD policy, release strategy, source tree preflight contract, Package.swift ownership
  contract and this source contract chain are updated.
- `docs/manual-intervention.md` documents that the current iOS upload workflow activation remains blocked-placeholder.
- `.github/workflows/ci.yml` checks this contract, Package.swift ownership contract, release workflow fields, manual marker and
  forbidden iOS artifact/source material.
