...
当前状态：blocked-placeholder。仓库仍不包含 `apps/ios` Swift source tree、Swift source、`Package.swift`、
Xcode project、Network Extension target、`PrivacyInfo.xcprivacy`、ExportOptions.plist、`.ipa`、`.xcarchive`、
`.xcresult`、dSYM bundle、Provisioning Profile、
...
- 固定 release workflow placeholder summary 必须输出 iOS upload workflow activation contract、marker 状态、protected
  ...
  asset blocked 字段。
- 固定 source tree preflight contract 必须先输出 `apps/ios`、`Package.swift`、Xcode project、Network Extension target、
  `PrivacyInfo.xcprivacy`、entitlement/provisioning、upload enabled marker 和 release/upload blocked 状态。
...
- The repository still has no `apps/ios` Swift source tree, `Package.swift`, Swift source, `PrivacyInfo.xcprivacy`,
  Xcode project, Network Extension target, ExportOptions.plist, `.ipa`,
...
- No `apps/ios` Swift source tree, Swift source, `Package.swift`, Xcode project, Network Extension target,
