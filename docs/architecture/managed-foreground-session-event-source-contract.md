...
本文定义 `v0.1.2-alpha.2` managed foreground lifecycle 的两个 source-only event record 切片和一个只读 CLI 接线切片。它只读取、初始写入并校验调用方显式指定的单个 JSON event record，不扫描目录、不聚合事件，也不把事件内容误称为进程存活或实时事件流。
...
event CLI 写入、事件列表/游标、实时 runtime event stream、日志读取、PID/port liveness 检查和 runtime control 由后续独立功能处理。
