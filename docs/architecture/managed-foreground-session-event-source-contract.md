...
本文定义 `v0.1.2-alpha.2` managed foreground lifecycle 的首个 source-only event record 读取切片。它只读取并校验调用方显式指定的单个 JSON event record，不扫描目录、不聚合事件、不产生事件，也不把事件内容误称为进程存活或实时事件流。
...
- `managed-foreground-session-event-cli=blocked`
...
event 写入、事件列表/游标、CLI 查询、实时 runtime event stream、日志读取、PID/port liveness 检查和 runtime control 由后续独立功能处理。
