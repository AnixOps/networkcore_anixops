...
manifest 诊断拒绝、manifest 错误拒绝审计、manifest 错误优先于权限拒绝、manifest 错误拒绝平台诊断保留、manifest 错误拒绝证书诊断保留、manifest 非错误诊断聚合、manifest 诊断权限拒绝保留、插件结果诊断聚合、平台诊断聚合、平台诊断拒绝保留、插件权限拒绝审计、审计事件聚合和插件端口错误传播。
