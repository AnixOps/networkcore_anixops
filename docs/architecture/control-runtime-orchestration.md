...
订阅 catalog runtime gate 已在 `RuntimeOrchestrator` 中落地，当前通过
`RuntimeSubscriptionCatalogGateResult`、`prepare_runtime_request_with_subscription_catalogs`、
`start_runtime_with_subscription_catalogs`、`reload_runtime_with_subscription_catalogs` 和显式
`SubscriptionService`/`SubscriptionSource` 把 inline `NodeCatalog.nodes` 编排进
`RuntimeConfigRequest.nodes`，用 `runtime.subscription.node_id_duplicate` 拒绝重复 id，
用 `runtime.subscription.rules_deferred` 保持 `NodeCatalog.rules` deferred，用
`runtime.subscription.source_unsupported` 拒绝非 inline source，并继续保持 no remote/file
subscription、no system DNS/TUN mutation、no daemon/control socket 边界。
`networkcore-linux start` 仍未暴露 subscription source。策略路由和 DNS 编排仍按后续扩展逐步加入。

## 后续扩展

- 将订阅 catalog 暴露到具体 CLI 或控制 API 前，必须继续沿用 [Subscription Catalog Runtime Orchestration Design](subscription-catalog-runtime-orchestration.md) 中的显式 source、重复 id 拒绝、rules deferred、no remote/file subscription 和 secret redaction 边界。
