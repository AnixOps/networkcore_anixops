...
`MitmPluginService`，覆盖平台 MITM 可用性、证书状态拒绝矩阵、证书诊断拒绝保留、远程脚本禁用与未知状态、
manifest 诊断拒绝、manifest 非错误诊断聚合、manifest 诊断权限拒绝保留、插件结果诊断聚合、平台诊断聚合、平台诊断拒绝保留、插件权限拒绝审计、审计事件聚合和插件端口错误传播。
