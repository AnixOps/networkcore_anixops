...
- Linux package artifact staging file validation contract 和 Linux package artifact archive creation validation contract 已定义；下一步可以补充 Linux package artifact checksum execution validation contract，明确真实 archive 创建后计算 `sha256`、写 archive checksum sidecar 和仍不写 manifest/upload 的边界。
