...
`v0.1.0-alpha.20` release hardening 让 `traffic-proof` text 输出显式打印 CONNECT authority，
方便不用 JSON 也能审计 proof 绑定。
...
发布边界：当前最新用户可下载 Linux artifact 是 `v0.1.0-rc.1`；本文描述 current `main`
源码合同。`v0.1.0-rc.1` 已覆盖 `verify --confirm`、`verify --confirm --target-url <url>`、`session-plan`、`--target-url`、
...
explicit SOCKS5 CONNECT-level plugin reject/proof 诊断、explicit HTTP proxy live plain HTTP data plane、TLS CA certificate PEM、private key PEM、dedicated profile CA PEM copy、explicit HTTP `CONNECT` pass-through tunnel foundation、bounded ClientHello/SNI observation、controlled downstream TLS termination plan/report、caller-provided HTTPS request rewrite preview，以及 caller-provided HTTPS response rewrite preview；rc.1 还固定 HTTPS request preview 不声称 live TLS decryption、live CONNECT-stream rewrite 或 JavaScript script dispatch。
当前 `main` 已同步到 `v0.1.0-rc.1` 发布边界；后续新能力仍必须等待新 tag、
...
- `BrowserCaptureTrafficProofProbe`、`LinuxBrowserCaptureTrafficProofRequest`、`LinuxBrowserCaptureTrafficProofReport`、`proof-log-token`、`proof_connect_authority`、text CONNECT authority 输出、`proof_target_url`、`networkcore_proof_token`、`MITM_BROWSER_CAPTURE_DEFAULT_PROOF_LOG_PATH`、`traffic_proof_command` 和 `traffic_proof_report` 作为 proof-log-token traffic proof anchor 可发现。
...
- `traffic-proof --confirm` 必须通过 `BrowserCaptureTrafficProofProbe` 注入执行，并由合同测试覆盖缺少授权、可选 `--target-url` 默认 proof 绑定、proof token observed、text CONNECT authority 输出、proof token/proxy/CONNECT authority binding mismatch、proof token missing、未接线 blocked 和 JSON `traffic_proof_report`。
