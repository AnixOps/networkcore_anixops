...
MITM_BROWSER_CAPTURE_GATE=pac-artifact-active/system-mutation-blocked
```
...
`mitm_status.browser_plan`、`browser_capture` 机器字段、脱敏 session-plan、显式授权的本地代理端点 verify、proof-log-token traffic proof，以及 NetworkCore-owned PAC artifact apply/rollback；但还没有用户可启用的 live browser capture，
也没有浏览器/系统代理写入、系统 PAC 安装、TUN/DNS/firewall mutation、CA lifecycle 或 HTTP/TLS data plane。
...
`traffic-proof --confirm --proof-token <token> --proof-log <path>` proof-log-token 验证入口，以及 `apply --confirm --pac-file <path> --snapshot <path>` / `rollback --snapshot <path>` PAC artifact apply/rollback 不在该 artifact 中；该 tag 之后的 main 增量需要后续新 tag release 通过 GitHub Actions
...
当前仓库源码只允许 pac-artifact-active/system-mutation-blocked 行为：
...
- `networkcore-linux mitm browser-capture launch --confirm [--browser <executable>] [--profile-dir <dir>] [--target-url <url>]` 通过注入的 `BrowserCaptureProcessRunner` 启动 dedicated browser profile，传入显式 `--proxy-server=http://127.0.0.1:7890`、`--user-data-dir=<dir>` 和可选 target URL 参数，并输出 `LinuxBrowserCaptureLaunchReport`；该命令不写系统代理、浏览器 policy、system PAC、TUN、DNS、firewall 或 CA 状态，也不验证 live browser capture。
- `networkcore-linux mitm browser-capture launch` 缺少 `--confirm` 时返回 authorization required，不调用 process runner。
- `networkcore-linux mitm browser-capture apply --confirm --pac-file <path> --snapshot <path>` 接受显式授权信号，通过 `BrowserCapturePacFileStore` 写入 operator-provided NetworkCore PAC 文件和 rollback snapshot。PAC 内容只返回 `PROXY 127.0.0.1:7890; DIRECT`，用于让调用方手动指向计划本地代理；该命令不安装系统 PAC，不写浏览器 policy，不写系统代理。缺少 `--pac-file` 或 `--snapshot` 时返回 config missing 且不写文件；已存在目标 PAC 或 snapshot 时拒绝覆盖。
- `networkcore-linux mitm browser-capture rollback --snapshot <path>` 读取 NetworkCore PAC snapshot，删除 snapshot 记录的 PAC 文件，并输出 rollback report；PAC 文件已不存在时按幂等成功处理。该命令不恢复或修改浏览器/system proxy/system PAC 状态。
...
- `browser_capture` 输出 action、gate、`BrowserCaptureAuthorization`、`BrowserCaptureRollbackSnapshot`、
  ...
  `BrowserCapturePacFileStore`、`LinuxBrowserCaptureApplyReport`、`LinuxBrowserCaptureRollbackReport`、verify report、traffic proof report 和 PAC artifact report。
- `MITM_BROWSER_CAPTURE_GATE` 保持 `pac-artifact-active/system-mutation-blocked`。
...
networkcore-linux mitm browser-capture apply --confirm --pac-file /tmp/networkcore-browser-capture.pac --snapshot /tmp/networkcore-browser-capture.snapshot.json
...
当前 PAC artifact apply 已满足：

- 调用方显式传入 `--confirm`。
- 调用方显式传入 `--pac-file <path>` 和 `--snapshot <path>`。
- `BrowserCaptureAuthorization` 记录授权来源、scope 和 gate。
- `BrowserCapturePacFileStore` 拒绝覆盖已存在 PAC 文件或已存在 rollback snapshot。
- rollback snapshot 只记录 NetworkCore 创建的 PAC artifact，rollback 只删除该 PAC 文件。
- snapshot 和 PAC 文件不得包含完整订阅 URL、cookie、token、私钥或浏览历史。
...
| `cli.linux.mitm.browser_capture.apply.ready` | Info | PAC artifact 和 rollback snapshot 已写入 operator-provided path，但不代表系统代理或浏览器 policy 已修改 |
...
| `cli.linux.mitm.browser_capture.rollback.ready` | Info | rollback 已根据 NetworkCore PAC snapshot 删除 PAC artifact |
| `cli.linux.mitm.browser_capture.rollback.failed` | Error | rollback 无法删除 snapshot 指向的 PAC artifact |
...
`traffic_proof_report`，PAC artifact `apply --confirm --pac-file <path> --snapshot <path>` 只写 NetworkCore PAC 文件和 rollback snapshot，`rollback --snapshot <path>` 只删除该 PAC 文件。系统/browser mutation、完整 live traffic verification、证书 lifecycle 和 HTTP/TLS data plane 仍必须等待后续源码实现并通过 GitHub Actions。
...
因此，PAC artifact apply 即使完成，也不得单独宣称 HTTPS MITM 可用；必须同时满足证书生命周期和 HTTP/TLS 数据面 gate。
...
- `MITM_BROWSER_CAPTURE_GATE` 当前仍是 `pac-artifact-active/system-mutation-blocked`。
...
- `BrowserCapturePacFileStore`、`CommandBrowserCapturePacFileStore`、`LinuxBrowserCapturePacRequest`、`LinuxBrowserCapturePacApplyOutcome`、`LinuxBrowserCapturePacRollbackOutcome`、`--pac-file`、`cli.linux.mitm.browser_capture.apply.ready`、`cli.linux.mitm.browser_capture.apply.config_missing`、`cli.linux.mitm.browser_capture.pac.write_failed`、`cli.linux.mitm.browser_capture.snapshot.read_failed` 和 `cli.linux.mitm.browser_capture.rollback.ready` 作为 PAC artifact apply/rollback anchor 可发现。
...
- `apply --confirm --pac-file <path> --snapshot <path>` 和 `rollback --snapshot <path>` 必须通过 `BrowserCapturePacFileStore` 注入执行，并由合同测试覆盖缺少 PAC/snapshot config、PAC artifact 写入、rollback、entrypoint routing 和 JSON artifact fields。
- 本机不得运行测试、构建、打包或发布验证。
