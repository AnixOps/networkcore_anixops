...
发布边界：当前最新用户可下载 Linux artifact 是 `v0.1.0`；本文描述 current `main`
源码合同。`v0.1.0` 已覆盖 `verify --confirm`、`verify --confirm --target-url <url>`、`session-plan`、`--target-url`、
...
当前 `main` 已同步到 `v0.1.0` 正式发布边界；后续新能力仍必须等待新 tag、
