...
- 在 license/NOTICE 人工确认、真实 packaging、attestation、
...
| `package_publish_eligibility_ci` | `active` |
...
| `ci` | release workflow 按 release CI gate activation validation contract、release CI gate execution validation contract 和 release CI gate API implementation plan 自动读取同 repository、同 commit、`main` 分支成功 CI run | `active` |
...
| `release_notes_rollback` | release notes/rollback execution gate 已校验 required fields、withdrawal 和 replacement policy | `blocked-not-generated` |

历史 placeholder 中，任何 blocked gate 都会让 aggregate status 保持 `blocked`。真实 publish
...
历史 placeholder release 不执行第 4 步及之后的任何真实 packaging、attestation、release notes、
...
- 标记 `linux-package-publish-eligibility-ci=active`。
...
- 本文档保持在 README、ROADMAP、Release Strategy、Linux artifact 设计、Linux package manifest
  ...
  validation contract、Linux package release notes/rollback policy binding contract、Linux package release
  notes/rollback execution validation contract、Linux CLI artifact 安装/回滚设计、Release CI success source contract、
  Linux package release CI gate activation validation contract、release CI gate execution validation contract、release CI gate API implementation plan、Linux package artifact job preflight validation
...
- Linux package license/NOTICE transition validation contract、Linux package release CI gate activation
  validation contract、release CI gate execution validation contract、release CI gate API implementation plan、Linux package artifact job preflight validation contract、Linux package artifact build
