...
explicit plain HTTP proxy live data plane 后必须遵守的源码边界。当前仓库允许调用方通过
...
reject、redirect、header mutation 和 body mutation。该能力不解密 TLS，不终止 HTTPS
CONNECT，不安装或信任 CA，不修改 browser/system proxy、system PAC、TUN、DNS 或 firewall，
也不执行插件脚本。
...
- `CONNECT` 或 `https://` target 在 HTTP listener 中返回 TLS blocked 诊断，不进入 TLS termination。
