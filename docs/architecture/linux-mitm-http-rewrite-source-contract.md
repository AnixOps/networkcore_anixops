...
gate 状态和合同测试。任何后续把 live TLS decryption、live CONNECT-stream HTTPS request/response rewrite、
HTTPS response rewrite、script runtime 或 browser/system capture mutation 从 blocked 改为 active 的提交，
必须先新增或修订 source contract，覆盖 CA trust、TLS interception、HTTP parser、body buffering、
script runtime、browser/system capture、rollback 和显式授权边界。
