...
- Release CI gate API implementation plan 已定义；下一步可以实现 `release-ci-gate` API read，启用
  `actions: read` 后按 plan 查询同 commit CI run、选择成功 run 并校验 `CI summary` job，同时继续阻止
  `package-linux` 和 GitHub Release asset。
