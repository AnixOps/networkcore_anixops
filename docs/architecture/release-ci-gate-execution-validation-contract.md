...
本文定义 release workflow 未来真正执行 `release-ci-gate` 时，如何自动读取同 repository、同
commit、`main` 分支成功 CI run，并在 `package-linux` 或任何 release asset 前失败阻断。当前仍是
placeholder；本文只固定 API 字段、权限、失败边界和 release workflow 占位输出，不启用 GitHub API
查询、不定义 `package-linux` job、不上传 artifact 或 release asset。
...
- 不实现真实 GitHub API 查询。
- 不启用 workflow `permissions: actions: read`。
...
| `release_ci_gate_execution_status` | `blocked-placeholder` |
| `release_ci_gate_execution_source` | `release-ci-gate` |
| `release_ci_gate_execution_current_mode` | `contract-only-no-api-read` |
| `release_ci_gate_execution_required_job` | `release-ci-gate` |
| `release_ci_gate_execution_job_status` | `placeholder` |
| `release_ci_gate_execution_required_permission` | `actions-read` |
| `release_ci_gate_execution_permission_status` | `not-enabled` |
| `release_ci_gate_execution_api_source` | `github-actions-runs-api` |
| `release_ci_gate_execution_api_status` | `not-called` |
...
| `release_ci_gate_execution_ci_summary` | `required-success` |
...
| `release_ci_gate_execution_next_action` | `ci-api-read-implementation-before-package-linux` |

`blocked-placeholder` 表示 release workflow 已记录真实 CI gate execution 的验证要求，但当前
`release-ci-gate` 仍不读取 GitHub API，也不证明 Linux artifact 可以进入 packaging。

## Future Gate Execution

未来真实 `release-ci-gate` 必须在 `release-policy` 后、`package-linux` 或任何 artifact job 前运行。
...
release-ci-gate:
  ...
  steps:
    - name: Validate same-commit CI success
      run: |
        # Future implementation only. Current workflow must not run this API query.
...
- API response 原文、token、Authorization header、runner 本地绝对路径、secret 或维护者私有身份被写入
  manifest、release notes、release summary 或 Step Summary。
- `package-linux`、`publish-github-release` 或 release asset upload 在 execution status 仍为
  `blocked-placeholder` 时被定义或执行。
...
- 标记 `release-ci-gate-execution-status=blocked-placeholder`。
- 标记 `release-ci-gate-execution-required-permission=actions-read`。
- 标记 `release-ci-gate-execution-permission-status=not-enabled`。
- 标记 `release-ci-gate-execution-api-status=not-called`。
...
- 标记 `release-ci-gate-execution-ci-summary=required-success`。
- 标记 `release-ci-gate-execution-package-linux=not-defined`。
- 标记 `release-ci-gate-execution-release-asset=blocked`。
- 标记 `release-ci-gate-execution-next-action=ci-api-read-implementation-before-package-linux`。
...
- 本文档保持在 README、ROADMAP、Release Strategy、CI policy、Release CI success source contract、
  Linux package release CI gate activation validation contract、Release CI gate API implementation plan、
  Linux package publish eligibility execution validation contract、Linux package publish eligibility aggregate contract、Linux package artifact job preflight
  validation contract、Linux package artifact manifest design、Linux artifact pre-release design 和 Linux CLI
  artifact 安装/回滚设计中可发现。
...
- `.github/workflows/release.yml` 的 `release-ci-gate` 检查本文档存在、标题，并输出 execution blocked 字段。
- `linux-artifact-readiness`、release placeholder 和 release summary 输出 execution status、required permission、
  API status、same release SHA requirement、required fields、CI summary、manual input blocked、`package-linux`
...
- 在 license/NOTICE 人工确认和 release CI gate API implementation 激活前，继续保持
  `release-ci-gate-execution-status=blocked-placeholder`。
- 下一步可以在单独增量中实现 release CI gate API read，按
  [Release CI Gate API Implementation Plan](release-ci-gate-api-implementation-plan.md) 启用 job-level
  `actions: read`、自动选择同 commit 成功 CI run、校验 CI summary job，并继续阻止 `package-linux` 和
