...
- 下一步可以补充 release CI gate API implementation plan，明确启用 `actions: read` 前的最小查询实现、
  API selection 规则、CI summary job 校验和 rollback 边界，并继续阻止 `package-linux` 和 GitHub Release asset。
