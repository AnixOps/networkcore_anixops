...
后续 activation validation contract 才能修改 release workflow placeholder summary；真实 upload job 仍必须等 iOS
source、signing、manual confirmation 和 protected environment 全部完成后独立启用。
...
- 相关 iOS readiness/design/source contract 指向本 upload workflow source contract，后续工作推进到 iOS upload
  workflow activation validation contract。
