...
- release placeholder 和 release summary 输出 `package-linux` runner/toolchain/target/crate/binary/platform input contract，以及 archive staging、文件来源和顶层目录组装合同，且继续说明 `package-linux` 未定义。
