...
- Release placeholder 和 release summary 已提前输出 manifest output contract 字段清单，真实 `package-linux` 后续必须提供对应值。
...
这些字段必须进入 release summary；没有 archive checksum、manifest 和 manifest checksum
...
- `linux-artifact-readiness` job 检查 Linux CLI 源码、platform adapter、native listener/node 配置设计、foreground stop/release 源码与合同测试、artifact manifest 合同设计、安装/回滚设计和 license/NOTICE 人工确认记录，并继续阻止 release asset。
- `release-placeholder` 和 release summary 输出 Linux artifact manifest output contract 字段清单，且继续说明没有 artifact job。
