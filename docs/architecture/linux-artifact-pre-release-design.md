...
- release placeholder 和 release summary 输出 `package-linux` runner/toolchain/target/crate/binary/platform input contract，以及 archive staging、文件来源、顶层目录组装、checksum/manifest checksum 文件命名、manifest 交叉校验、workflow artifact retention、publish download source、release asset set、禁止覆盖、signing policy、attestation/provenance policy、release notes/rollback policy、withdrawal/replacement policy、publish eligibility aggregate status 和未启用 blocked 合同，且继续说明 `package-linux`、`attest-linux`、`publish-github-release` 与 `post-release-summary` 未定义。
...
- 下一步可以补充 Linux package license/NOTICE confirmed-state transition validation contract，仍不生成 artifact。
