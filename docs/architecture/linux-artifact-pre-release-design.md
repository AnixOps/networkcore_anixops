...
本文件定义首个 Linux release artifact 进入 `.github/workflows/release.yml` 前必须满足的源码、平台设计、CI 和发布契约。它不是当前可发布产物说明；在本文门禁完成前，release workflow 仍只能保持 placeholder 状态。
...
- 定义 Linux packaging job 必须输出的 artifact、checksum、签名/证明和回滚字段。
- 避免在没有可运行入口、安装/卸载策略和 GitHub Actions 验证前发布空壳产物。

## 非目标

- 不在本阶段实现 Linux CLI、daemon、systemd unit、TUN 配置或安装器。
...
- Linux 平台 adapter 设计文档，定义 TUN、权限、DNS、服务管理、证书和诊断边界。
- 可运行入口源码，例如后续 `apps/linux-cli`、`apps/linux-daemon` 或等价 crate，并明确依赖 `control-runtime` 的方向。
...
这些字段必须进入 release summary；没有这些字段不得上传 Linux artifact。
...
6. `package-linux`
7. 后续可选 `sign-linux` 或 attestation job
8. `publish-github-release`
9. `post-release-summary`

`package-linux` 必须在 GitHub-hosted `ubuntu-latest` 或后续受控 Linux runner 中运行。所有 build、test、package、checksum、attestation 和 upload 都必须在 GitHub Actions 中完成。
...
- Linux adapter 设计文档完成并通过 CI governance 检查。
- Linux 可运行入口源码和对应 GitHub Actions 验证存在。
- `package-linux` job 输出 artifact、checksum、签名/证明状态和回滚字段。
- release summary 门禁 `package-linux`、checksum、签名/证明和回滚字段。

## 后续工作

- 补充 Linux platform adapter 设计文档，定义 TUN、权限、DNS 和服务管理能力探测边界。
- 在出现 Linux 可运行入口前，release workflow 不得生成 Linux artifact。
- 首个 artifact job 加入时同步更新 `docs/release-strategy.md`、`README.md`、`CHANGELOG.md` 和 `TODO.md`。
