...
- release placeholder 和 release summary 输出 `package-linux` runner/toolchain/target/crate/binary/platform input contract，以及 archive staging、文件来源、顶层目录组装、checksum/manifest checksum 文件命名和 manifest 交叉校验合同，且继续说明 `package-linux` 未定义。
...
- 下一步可以补充 `package-linux` publish/upload boundary、workflow artifact retention 和 release asset 阻断合同，仍不生成 artifact。
