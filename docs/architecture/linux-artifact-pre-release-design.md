...
- [Release CI success source contract](release-ci-success-source-contract.md) 完成并通过 CI governance 和 `release-ci-gate` 检查；当前 `release-ci-gate` 已通过 GitHub Actions API 输出 active CI source fields。
- [Linux package release CI gate activation validation contract](linux-package-release-ci-gate-activation-validation-contract.md)、[Release CI gate execution validation contract](release-ci-gate-execution-validation-contract.md) 和 [Release CI Gate API Implementation Plan](release-ci-gate-api-implementation-plan.md) 完成并通过 CI governance、`release-ci-gate`、`linux-artifact-readiness`、release placeholder 和 release summary 检查；当前输出 active activation/execution/API implementation 字段，但仍不定义 `package-linux`。
...
- [Linux package release notes/rollback execution validation contract](linux-package-release-notes-rollback-execution-validation-contract.md) 完成并通过 CI governance、`linux-artifact-readiness`、release placeholder 和 release summary 检查；当前仍只输出 blocked-placeholder execution 字段，不定义 `post-release-summary`、不创建 GitHub Release。
...
- [Linux package publish eligibility execution validation contract](linux-package-publish-eligibility-execution-validation-contract.md) 完成并通过 CI governance、`linux-artifact-readiness`、release placeholder 和 release summary 检查；当前仍只输出 blocked-placeholder execution 字段，不定义 `publish-eligibility-gate`、`publish-github-release` 或 release asset。
...
- `release-ci-gate` 和 release summary 输出 release CI success source contract、release CI gate activation validation contract、release CI gate execution validation contract、release CI gate API implementation plan 与 required fields，且继续说明真实 artifact 前必须关联 `main` 上同 commit 成功 CI。
- release placeholder 和 release summary 输出 `package-linux` preflight blocked status、artifact build command blocked status、artifact staging file blocked status、runner/toolchain/target/crate/binary/platform input contract，以及 release CI gate activation validation status、release CI gate execution validation status、archive staging、文件来源、顶层目录组装、checksum/manifest checksum 文件命名、manifest 交叉校验、workflow artifact retention、publish download source、release asset set、禁止覆盖、signing policy、attestation/provenance policy、release notes/rollback policy、release notes/rollback execution status、publish eligibility execution status、withdrawal/replacement policy、publish eligibility aggregate status、license/NOTICE transition validation status 和未启用 blocked 合同，且继续说明 `package-linux`、`attest-linux`、`publish-eligibility-gate`、`publish-github-release` 与 `post-release-summary` 未定义。
