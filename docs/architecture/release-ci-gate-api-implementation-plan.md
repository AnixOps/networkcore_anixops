...
本文定义 `release-ci-gate` 从 placeholder 进入真实 GitHub Actions API 读取前的最小实现计划。当前仍是
plan-only 阶段；本文只固定后续 API endpoint、查询参数、run 选择规则、CI summary job 校验、输出字段和失败回滚边界，
不启用 `actions: read`，不调用 GitHub API，不定义 `package-linux`，不上传 workflow artifact 或 release asset。

评估时间：2026-07-07。

## 目标

- 明确后续 `release-ci-gate` 的第一步实现只读取同 repository、同 commit、`main` 分支的 `CI` workflow run。
- 固定 GitHub Actions workflow runs API 和 workflow jobs API 的最小使用方式。
- 定义成功 run 的选择规则、CI summary job 成功校验和 machine-readable 输出字段。
- 保持实现顺序可回滚：API read 激活必须早于 `package-linux`、workflow artifact upload、attestation 和 GitHub Release asset。
- 防止 maintainer 用手写 run id、run URL、Step Summary 文本、聊天记录或本地命令绕过自动校验。

## 非目标

- 不在本增量启用 workflow `permissions: actions: read`。
- 不在本增量调用 GitHub Actions API。
...
当前 plan-only placeholder 固定为：

| 字段 | 值 |
| --- | --- |
| `release_ci_gate_api_implementation_plan` | `present` |
| `release_ci_gate_api_implementation_status` | `planned-blocked` |
| `release_ci_gate_api_implementation_source` | `release-ci-gate` |
| `release_ci_gate_api_implementation_current_mode` | `plan-only-no-api-read` |
| `release_ci_gate_api_implementation_required_permission` | `actions-read` |
| `release_ci_gate_api_implementation_permission_status` | `not-enabled` |
...
| `release_ci_gate_api_implementation_summary_status` | `required-success` |
...
| `release_ci_gate_api_implementation_next_action` | `release-ci-gate-api-read-implementation-after-plan` |

`planned-blocked` 表示仓库已经固定 API implementation plan，但 release workflow 仍保持 no-api-read 模式，不证明
Linux artifact 可以进入 packaging。

## Future API Query

后续第一版真实实现必须只在 `release-ci-gate` job 内加入 API read，并保持 `package-linux` 未定义。该 job 必须在
...
7. 输出 required CI source fields，供后续 `package-linux`、manifest、release notes 和 release summary 消费。
8. 只记录整理后的字段，不记录 API response 原文。

## Future Output Fields

API read 成功后，`release-ci-gate` 必须把当前 placeholder 字段切换为 active 字段：
...
- 在 `release-ci-gate`、`linux-artifact-readiness`、release placeholder 和 release summary 中输出 API implementation plan。
- 标记 `release-ci-gate-api-implementation-plan=present`。
- 标记 `release-ci-gate-api-implementation-status=planned-blocked`。
- 标记 `release-ci-gate-api-implementation-current-mode=plan-only-no-api-read`。
- 标记 `release-ci-gate-api-implementation-required-permission=actions-read`。
- 标记 `release-ci-gate-api-implementation-permission-status=not-enabled`。
...
- 标记 `release-ci-gate-api-implementation-summary-status=required-success`。
...
- 标记 `release-ci-gate-api-implementation-next-action=release-ci-gate-api-read-implementation-after-plan`。
- 继续不调用 GitHub Actions API、GitHub Releases API、`gh release create`、release action、upload-release-asset API 或
...
- `.github/workflows/release.yml` 的 `release-ci-gate` 检查本文档存在、标题，并输出 API implementation
  plan-only 字段。
...
- 下一步可以实现 `release-ci-gate` API read，启用 `actions: read` 并按本文档查询同 commit 成功 CI run。
