...
artifact manifest JSON 前必须满足的 manifest generation 验证合同。历史 placeholder 阶段曾是 contract-only；
...
`blocked-placeholder` 表示 release workflow 已记录未来 manifest generation 的验证要求，但历史 placeholder
...
- 历史 placeholder 不生成 artifact、不定义 `package-linux`、不定义 `publish-github-release`、不创建 archive、
