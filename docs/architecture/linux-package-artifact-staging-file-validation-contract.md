...
staging file 验证合同。历史 placeholder 阶段曾是 contract-only；本文只固定 build output、INSTALL、
...
`blocked-placeholder` 表示 release workflow 已记录未来 staging file 的验证要求，但历史 placeholder
