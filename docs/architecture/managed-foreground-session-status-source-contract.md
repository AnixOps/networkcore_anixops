...
- `managed-foreground-session-status-rollback-operation=status-rollback`
...
第四个源码增量提供 `ManagedForegroundSessionStatusRollbackRequest`、
`ManagedForegroundSessionStatusRollbackReport` 和 `CommandManagedForegroundSessionStore::rollback_status`。
调用方必须显式提供当前 status record 路径、已存在的 snapshot 路径和 expected state，且两个路径必须不同。
回滚先读取并验证当前 record 与 snapshot 的 schema version 1 和字段；只有当前 recorded state 与 trim 后的
expected state 相同，且 snapshot 的 trim 后 session id/engine id 与当前 record 相同才会继续。它将 snapshot 的
...
第六个源码增量将初始写入能力接入 `networkcore-linux managed-status init <status-record-path> <session-id> <engine-id> <state>`。
...
第七个源码增量将 expected-state 迁移能力接入 `networkcore-linux managed-status transition <status-record-path> <snapshot-path> <expected-state> <next-state>`。
...
第四个合同测试必须证明一次 `rollback_status`：

- 当 explicit `running` record 与 session/engine 匹配的 explicit `starting` snapshot 同时存在时，将 snapshot 原始内容复原到 status record；
- report 固定恢复前后 state、`snapshot_retained=true` 与 `liveness_verified=false`，且 snapshot 保持不变；
- stale expected state 返回稳定 state-conflict 错误，不修改 status record 或 snapshot。

第五个合同测试必须证明 `managed-status`：
...
第六个合同测试必须证明 `managed-status init`：
...
第七个合同测试必须证明 `managed-status transition`：
