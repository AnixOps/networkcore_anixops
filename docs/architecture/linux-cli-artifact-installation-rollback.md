...
- `release-ci-gate` 输出 release CI success source contract、release CI gate execution validation contract、release CI gate API implementation plan 和 required CI run/source 字段，真实 `package-linux` 前必须替换为同 commit 成功 CI run 自动读取门禁。
