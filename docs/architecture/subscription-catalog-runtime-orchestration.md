...
- `config-core::CoreSubscriptionService`，当前只接受显式 `inline:` source，把最小 subscription TOML `nodes`/`routes` 子集、URL/list/base64 list parser gates、Clash YAML parser gate、sing-box JSON parser gate、Surge proxy line parser gate 和 Loon proxy line parser gate 解析为 `SubscriptionDocument` 并归一化为 `NodeCatalog`。
