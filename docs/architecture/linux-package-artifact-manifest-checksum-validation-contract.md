...
manifest checksum sidecar 前必须满足的验证合同。历史 placeholder 阶段曾是 contract-only；本文只固定
...
`blocked-placeholder` 表示 release workflow 已记录未来 manifest checksum 的验证要求，但历史 placeholder
