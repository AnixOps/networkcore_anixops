...
- Linux package artifact manifest generation validation contract 已定义；下一步可以补充 Linux package artifact manifest checksum validation contract，明确真实 manifest JSON 生成后计算 manifest sha256、写 manifest checksum sidecar 和仍不 upload 的边界。
