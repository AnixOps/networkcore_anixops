...
- 下一步可以补充 Linux package artifact manifest generation validation contract，明确真实 archive checksum sidecar 写入后生成 manifest JSON 和仍不计算 manifest checksum/upload 的边界。
