...
当前仍不提交 `.entitlements` 文件。后续如果需要非 secret entitlement 文件，必须先补充独立 signing/entitlement
source contract，证明文件只包含最小 capability 声明，并且所有证书、profile、Team ID、App Store Connect
凭据仍只能来自 GitHub Secrets、GitHub Environments 或 Apple 官方平台。
