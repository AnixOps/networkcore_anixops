...
- 为后续 Swift/Xcode source contract 和 GitHub Actions `macos-26` 验证入口提供验收锚点。
...
后续出现 Swift package、Xcode project 或 Network Extension target 时，必须先补充 source contract，并且只在 GitHub
...
- Swift/Xcode source contract 已完成。
