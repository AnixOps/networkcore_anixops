...
| `networkcore-linux mitm status` / `networkcore-linux mitm diagnostics` / `networkcore-linux mitm certificate-plan` / `networkcore-linux mitm certificate apply/rollback` / `networkcore-linux mitm browser-plan` | 输出 MITM policy-only 状态、插件策略加载状态、证书生命周期计划、certificate artifact lifecycle report、浏览器捕获计划和 deferred gate 诊断 | `mitm-cli-command-gate-status=partial-active`；`MITM_CERTIFICATE_LIFECYCLE_GATE` 为 artifact-lifecycle-active/profile-trust-artifact-active/trust-mutation-blocked，只写 operator-provided CA certificate PEM、private key PEM、可选 dedicated profile CA PEM copy 和 snapshot，不安装或信任 CA，不写 system/browser trust store 或 profile trust state；`MITM_BROWSER_CAPTURE_GATE` 为 pac-policy-profile-prefs-active/system-mutation-blocked；不解密 HTTPS，不写入浏览器/系统代理 |
| `networkcore-linux mitm http-rewrite plan/preview` | 输出 Linux MITM HTTP rewrite source contract report、caller-provided plain/HTTPS rewrite request、authorization 和 outcome | `MITM_HTTP_TLS_DATA_PLANE_GATE` 为 plain-http-live-data-plane-active/tls-decryption-blocked；`preview --confirm --url <url>` 把 plugin outcome 应用到调用方提供的 plain HTTP input，并可在 current `main` 对 caller-provided request/response-phase `https://` input 预览 request reject/redirect/header mutation 与 response header/body mutation；native `ListenerKind::Http` explicit proxy path 对真实 `http://` HTTP/1.x request/response 应用 reject、redirect、header/body rewrite，并可对 explicit HTTP `CONNECT host:443` 建立 pass-through tunnel foundation 和 bounded ClientHello/SNI observation；仍不解密 TLS、不执行 live CONNECT 后 HTTPS request/response rewrite、不执行完整 live HTTPS response rewrite、不执行 script dispatch、不修改系统状态 |
