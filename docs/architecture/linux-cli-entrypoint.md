...
| `networkcore-linux mitm http-rewrite plan/preview` | 输出 Linux MITM HTTP rewrite source contract report、caller-provided plain/HTTPS rewrite request、authorization 和 outcome | `MITM_HTTP_TLS_DATA_PLANE_GATE` 为 plain-http-live-data-plane-active/tls-decryption-blocked；`preview --confirm --url <url>` 把 plugin outcome 应用到调用方提供的 plain HTTP input，并可在 current `main` 对 caller-provided request-phase `https://` input 预览 reject、redirect 和 request header mutation；native `ListenerKind::Http` explicit proxy path 对真实 `http://` HTTP/1.x request/response 应用 reject、redirect、header/body rewrite，并可对 explicit HTTP `CONNECT host:443` 建立 pass-through tunnel foundation 和 bounded ClientHello/SNI observation；仍不解密 TLS、不执行 live CONNECT 后 HTTPS request/response rewrite、不执行 HTTPS response rewrite、不执行 script dispatch、不修改系统状态 |
