...
- `handle_entrypoint` 将 `capabilities`、`status`、`diagnostics`、`mitm status/diagnostics/certificate-plan/browser-plan`、browser-capture 的 plan/launch-plan/session-plan 和 blocked apply/rollback/verify 路由到注入的 `PlatformCapabilityService`；`handle_entrypoint_with_browser_capture_io` 将 `mitm browser-capture launch --confirm` 路由到注入的 `BrowserCaptureProcessRunner`，并将 `mitm browser-capture verify --confirm` 路由到注入的 `BrowserCaptureEndpointProbe`；二进制入口使用 `ReadOnlyLinuxPlatformCapabilityService<HostLinuxReadOnlyProbe>`、`CommandBrowserCaptureProcessRunner` 和 `CommandBrowserCaptureEndpointProbe`。
