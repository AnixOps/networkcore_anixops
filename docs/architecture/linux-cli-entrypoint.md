...
| `networkcore-linux mitm http-rewrite plan/preview` | 输出 Linux MITM HTTP rewrite source contract report、caller-provided plain HTTP rewrite request、authorization 和 outcome | `MITM_HTTP_TLS_DATA_PLANE_GATE` 为 plain-http-rewrite-foundation-active/tls-decryption-blocked；`preview --confirm --url <url>` 只把 plugin outcome 应用到调用方提供的 plain HTTP input，不解密 TLS、不拦截 live traffic、不执行 script dispatch、不修改系统状态 |
