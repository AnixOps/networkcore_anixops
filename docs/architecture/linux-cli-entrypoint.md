...
| `networkcore-linux mitm status` / `networkcore-linux mitm diagnostics` | 输出 MITM policy-only 状态、插件策略加载状态和 deferred gate 诊断 | `mitm-cli-command-gate-status=partial-active`；不生成 CA，不解密 HTTPS，不执行 browser hijack |

首个源码增量已实现命令骨架和测试替身；`capabilities`、`status`、`diagnostics` 和 `mitm status/diagnostics` 已通过 `PlatformCapabilityService` 接入 `HostLinuxReadOnlyProbe`。代理引擎执行、daemon/control socket、CA lifecycle、browser hijack 和任何会修改系统状态的 Linux 能力必须等对应 adapter 设计和 CI 验证完成后再接入。
...
- `handle_capabilities`、`handle_status`、`handle_diagnostics`、`handle_mitm_status`、`handle_stop` 和 JSON renderer 覆盖平台诊断、MITM policy-only status、无 daemon stop、无 runtime context status 和自动化输出合同。
- `handle_entrypoint` 将 `capabilities`、`status`、`diagnostics` 和 `mitm status/diagnostics` 路由到注入的 `PlatformCapabilityService`；二进制入口使用 `ReadOnlyLinuxPlatformCapabilityService<HostLinuxReadOnlyProbe>`。
