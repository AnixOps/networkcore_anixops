...
| `networkcore-linux start --config <path>` | 前台启动 runtime，用当前进程持有生命周期；native engine 注入内置 `networkcore.adblock` MITM hook | 不默认 daemonize，不安装服务，不后台运行；默认只在 SOCKS5 CONNECT 层应用插件 `Reject` 为 CONNECT failure。只有显式 `--enable-https-mitm --mitm-ca-cert <path> --mitm-ca-key <path> --confirm` 才启用受控 HTTP/1.1 TLS termination/rewrite，且不支持 HTTP/2、HTTP/3/QUIC 或 pinning bypass |
...
| `networkcore-linux mitm http-rewrite plan/preview` | 输出 Linux MITM HTTP rewrite source contract report、caller-provided plain/HTTPS rewrite request、authorization 和 outcome | `MITM_HTTP_TLS_DATA_PLANE_GATE=plain-http-live-data-plane-active/tls-decryption-blocked` 仅描述 preview；`preview --confirm --url <url>` 继续不解密 TLS 或执行脚本。独立的显式 `start --enable-https-mitm --mitm-ca-cert <path> --mitm-ca-key <path> --confirm` 在 CONNECT authority/SNI 一致时提供受控 HTTP/1.1 TLS termination、web-PKI upstream forwarding 与最多四个 request/response rewrite exchange；HTTP/2、HTTP/3/QUIC、pinning bypass、远程脚本和系统状态 mutation 不支持 |
| `networkcore-linux mitm browser-capture plan/launch-plan/session-plan/launch/apply/rollback/verify/traffic-proof` | 输出浏览器捕获 source contract report、manual dedicated-profile launch templates、脱敏订阅到本地代理/浏览器/verify session plan、可选 target URL、dedicated-profile launch report、`proxy_scheme`、本地代理端点/target route verify report、proof-log-token traffic proof report、PAC/browser policy/profile prefs artifact apply/rollback report、显式授权记录、snapshot 路径记录和 blocked diagnostics | `session-plan <ss://url>` 只输出脱敏命令计划，可通过 `--target-url <url>` 把目标页面写入 dedicated browser command 和 verify command，可通过 `--proxy-scheme socks5` 把 dedicated browser/PAC/policy/probe 计划绑定到 `socks5://127.0.0.1:7890` 以显式走 native SOCKS5 CONNECT hook，不启动 `sing-box` 或浏览器；`launch --confirm` 只通过注入 runner 启动带显式代理参数的 dedicated browser profile，可把 target URL 作为浏览器参数打开；`verify --confirm` 只通过注入 endpoint probe 探测计划本地代理端点，传入 `--target-url <url>` 时只探测目标 URL 代理通路；`traffic-proof --confirm [--target-url <url>] [--proof-token <token>] [--proof-log <path>]` 只通过注入 proof probe 读取 operator-provided proof log 并检查 token，可在 token/log 省略时复用默认 proof 绑定；`apply --confirm --pac-file <path> [--policy-file <path>] [--profile-prefs-file <path>] --snapshot <path>` 只写 operator-provided NetworkCore PAC artifact、可选 Chromium/Chrome managed proxy policy artifact、可选 Firefox dedicated profile prefs、`profile_prefs_file_path`/`profile_prefs_content` 和 rollback snapshot，`rollback --snapshot <path>` 只回滚这些 artifact；不写系统代理、不安装 browser policy、system PAC、TUN、DNS、firewall 或 CA 状态 |

首个源码增量已实现命令骨架和测试替身；`capabilities`、`status`、`diagnostics`、`mitm status/diagnostics/certificate-plan/certificate apply/rollback/browser-plan`、`mitm http-rewrite plan/preview` 和 `mitm browser-capture plan/launch-plan/session-plan/launch/apply/rollback/verify/traffic-proof` 已通过 `PlatformCapabilityService` 接入 `HostLinuxReadOnlyProbe`。`start` 已通过 `RuntimeOrchestrator` 接入 `engine-native::NativeProxyEngineService` 和前台 lifecycle host，并通过 `native_proxy_engine_service_with_builtin_mitm_plugin` 注入内置 `networkcore.adblock` hook；插件返回 `Reject` 时只在 SOCKS5 CONNECT 层写 general failure response。`certificate-plan` 输出当前证书状态、artifact lifecycle 步骤、trust blocked operations 和 `mutation_ready=false`；`certificate apply/rollback` 通过 `CommandMitmCertificateArtifactStore` 写入或删除 operator-provided CA certificate PEM、private key PEM、可选 dedicated profile CA PEM copy 和 snapshot，输出 `certificate_lifecycle` report，但不安装或信任 CA；`http-rewrite` 输出 `http_rewrite` report、`LinuxMitmHttpRewriteReport`、显式授权和 blocked operations，`preview` 通过 `NativePlainHttpMessage`/`NativePlainHttpRewriteReport` 应用 caller-provided plain HTTP rewrite outcome，并在 current `main` 通过 `NativeHttpsRequestRewritePreviewReport` 和 `NativeHttpsResponseRewritePreviewReport` 表达 caller-provided HTTPS request reject/redirect/header mutation preview 与 response header/body mutation preview；native `ListenerKind::Http` explicit proxy path 通过 `NativeExplicitHttpProxyRequest` 和 `NativePlainHttpProxyResponse` 对真实 `http://` HTTP/1.x request/response 应用 reject、redirect、header/body rewrite，并通过既有 SOCKS outbound primitive 转发非 terminal request；current `main` 还可对 explicit HTTP `CONNECT host:443` 建立标准 `HTTP/1.1 200 Connection Established` pass-through tunnel foundation，并 bounded-peek TLS ClientHello/SNI，但不做 downstream TLS termination、HTTPS decryption、live CONNECT 后 HTTPS request/response rewrite 或完整 live HTTPS response rewrite；`browser-plan` 只输出当前捕获状态、默认显式代理计划 `127.0.0.1:7890`、计划步骤、blocked operations 和 `mutation_ready=false`；`browser-capture` 输出 `browser_capture` report、manual launch templates、`LinuxBrowserCaptureSessionPlanReport`、`LinuxBrowserCaptureLaunchReport`、`LinuxBrowserCaptureVerifyReport`、`LinuxBrowserCaptureTrafficProofRequest`、`LinuxBrowserCaptureTrafficProofReport`、`LinuxBrowserCapturePacRequest`、`BrowserCapturePacFileStore`、`proxy_scheme`、可选 target URL、`--proxy-scheme socks5` native plugin proxy mode、`--policy-file` browser policy artifact、`--profile-prefs-file` Firefox dedicated profile prefs artifact、`profile_prefs_file_path`、`profile_prefs_content`、`probe=http-connect-target` target route verify、`probe=proof-log-token` traffic proof、PAC/browser policy/profile prefs artifact apply/rollback、`BrowserCaptureAuthorization` 和 `BrowserCaptureRollbackSnapshot` 边界。daemon/control socket、system trust store mutation lifecycle、browser/system proxy mutation、browser hijack、system PAC 安装、HTTPS 解密、live HTTPS rewrite、script dispatch 执行和任何会修改系统状态的 Linux 能力必须等对应 adapter 设计和 CI 验证完成后再接入。

当前 `main` 的受控 TLS 增量覆盖上述历史 preview 限制：`start --enable-https-mitm --mitm-ca-cert <path> --mitm-ca-key <path> --confirm` 在 authority/SNI 一致时执行 authority-bound leaf、下游 TLS termination、web-PKI upstream forwarding 和最多四个有界 HTTP/1.1 request/response rewrite exchange。该路径不改变 preview report 的 `tls-decryption-blocked` 状态，也不支持 HTTP/2、HTTP/3/QUIC、certificate/public-key pinning bypass、远程脚本、browser/system proxy mutation 或无界 streaming。

当前 Linux managed-service 入口统一为显式的 `connect`、`disconnect`、`restart`、`status`、`logs`、`diagnostics` 和 `rollback` 表面：`connect --config <path>` 保持前台 runtime；带 `--confirm` 或 `--service-unit <name>` 的 `connect`/`disconnect`/`restart` 分别映射到命名 systemd unit 的 start/stop/restart，且 systemd mutation 仍要求 `--confirm`。`status --service-unit <name>` 只读取该 unit；plain `status` 不推断后台 runtime。`logs` 是 `managed-log` 的别名，只读取显式文件的有界尾部。runtime `rollback` 仅通过显式 managed control socket、expected config version 和 `--confirm` 请求当前 session，绝不扫描默认 socket 或 PID。`diagnostics` 保持聚合且脱敏的只读输出。
...
该 crate 当前执行只读 Linux 能力探测、只读配置准备、前台 native runtime 和显式受确认的 managed-service 操作；不会发现或控制任意 daemon。Linux artifact 发布路径已由 GitHub Actions release workflow 打通，CLI crate 仍不在本机打包或发布。Unix 默认前台 host 会监听 `SIGINT`/`SIGTERM` 并映射为 `cli.linux.start.signal_received`、`cli.linux.start.lifecycle_interrupted` 和 130 退出码；interruption 后会通过当前进程内 `RuntimeOrchestrator::stop_runtime` 释放 native runtime 并聚合 `engine.native.runtime.accept_loop_stopped`/`engine.native.runtime.released` 诊断，stop 失败时追加 `cli.linux.start.runtime_stop_failed`。同一 host 还会消费 explicit managed control socket 的已确认 stop 请求；plain `stop`、plain `status` 和未指定 control socket 的 `rollback` 仍保持 stable unavailable 或 blocked 边界，显式 named systemd unit 的 start/stop/restart/reload/status 则由 platform adapter 管理。
