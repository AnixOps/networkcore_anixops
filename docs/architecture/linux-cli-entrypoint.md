...
| `networkcore-linux mitm http-rewrite plan/preview` | 输出 Linux MITM HTTP rewrite source contract report、caller-provided plain/HTTPS rewrite request、authorization 和 outcome | `MITM_HTTP_TLS_DATA_PLANE_GATE` 仅描述 preview；`preview --confirm --url <url>` 继续不解密 TLS 或执行脚本。独立的显式 `start --enable-https-mitm --mitm-ca-cert <path> --mitm-ca-key <path> --confirm` 在 CONNECT authority/SNI 一致时提供受控 HTTP/1.1 TLS termination、web-PKI upstream forwarding 与最多四个 request/response rewrite exchange；HTTP/2、HTTP/3/QUIC、pinning bypass、远程脚本和系统状态 mutation 不支持 |
