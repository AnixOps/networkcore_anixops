...
首个源码增量已实现命令骨架和测试替身；`capabilities`、`status`、`diagnostics` 和 `mitm status/diagnostics/certificate-plan/browser-plan` 已通过 `PlatformCapabilityService` 接入 `HostLinuxReadOnlyProbe`。`certificate-plan` 只输出当前证书状态、计划步骤、blocked operations 和 `mutation_ready=false`；`browser-plan` 只输出当前捕获状态、默认显式代理计划 `127.0.0.1:7890`、计划步骤、blocked operations 和 `mutation_ready=false`。代理引擎执行、daemon/control socket、CA mutation lifecycle、browser capture mutation、browser hijack 和任何会修改系统状态的 Linux 能力必须等对应 adapter 设计和 CI 验证完成后再接入。
...
- `handle_capabilities`、`handle_status`、`handle_diagnostics`、`handle_mitm_status`、`handle_mitm_certificate_plan`、`handle_mitm_browser_plan`、`handle_stop` 和 JSON renderer 覆盖平台诊断、MITM policy-only status、certificate plan、browser plan、无 daemon stop、无 runtime context status 和自动化输出合同。
