...
| `networkcore-linux mitm status` / `networkcore-linux mitm diagnostics` / `networkcore-linux mitm certificate-plan` | 输出 MITM policy-only 状态、插件策略加载状态、证书生命周期计划和 deferred gate 诊断 | `mitm-cli-command-gate-status=partial-active`；`MITM_CERTIFICATE_LIFECYCLE_GATE` 仅 plan-only，不生成 CA，不解密 HTTPS，不执行 browser hijack |

首个源码增量已实现命令骨架和测试替身；`capabilities`、`status`、`diagnostics` 和 `mitm status/diagnostics/certificate-plan` 已通过 `PlatformCapabilityService` 接入 `HostLinuxReadOnlyProbe`。`certificate-plan` 只输出当前证书状态、计划步骤、blocked operations 和 `mutation_ready=false`。代理引擎执行、daemon/control socket、CA mutation lifecycle、browser hijack 和任何会修改系统状态的 Linux 能力必须等对应 adapter 设计和 CI 验证完成后再接入。
...
- JSON 输出必须包含 `ok`、`command`、`diagnostics`，有 runtime 结果时包含 `platform` 和 `engine_status`；MITM 状态命令必须包含 `mitm_status`。
...
- `handle_capabilities`、`handle_status`、`handle_diagnostics`、`handle_mitm_status`、`handle_mitm_certificate_plan`、`handle_stop` 和 JSON renderer 覆盖平台诊断、MITM policy-only status、certificate plan、无 daemon stop、无 runtime context status 和自动化输出合同。
- `handle_entrypoint` 将 `capabilities`、`status`、`diagnostics` 和 `mitm status/diagnostics/certificate-plan` 路由到注入的 `PlatformCapabilityService`；二进制入口使用 `ReadOnlyLinuxPlatformCapabilityService<HostLinuxReadOnlyProbe>`。
