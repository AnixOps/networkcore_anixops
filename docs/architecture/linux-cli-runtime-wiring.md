...
- `config-core` 已提供最小纯配置服务和最小 inline subscription parser；[Subscription Catalog Runtime Orchestration Design](subscription-catalog-runtime-orchestration.md) 已在 `control-runtime` 落地显式 subscription catalog runtime gate，但 `networkcore-linux prepare-config/start` 当前仍未消费 subscription catalog，也不扫描默认订阅路径。后续配置和订阅扩展继续保持纯内存解析、无隐式网络/文件读取和 secret 不泄露边界。
...
- 后续新增 daemon discovery、默认 control socket、DNS/TUN mutation 或其他系统级能力前，继续先补设计、显式授权、snapshot 和 rollback 合同；当前 named systemd service install/control/removal 与 release artifact 已有独立的 source/release contracts。
