...
- `config-core` 已提供最小纯配置服务和最小 inline subscription parser；[Subscription Catalog Runtime Orchestration Design](subscription-catalog-runtime-orchestration.md) 已定义 `NodeCatalog` 进入 `RuntimeConfigRequest.nodes` 前的运行层 gate，但 `networkcore-linux prepare-config/start` 当前仍未消费 subscription catalog。后续配置和订阅扩展继续保持纯内存解析、无隐式网络/文件读取和 secret 不泄露边界。
