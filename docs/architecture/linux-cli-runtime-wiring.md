...
- `config-core` 已提供最小纯配置服务和最小 inline subscription parser；`networkcore-linux prepare-config` 已接入二进制入口，subscription catalog 进入 runtime request 仍属于 P3 后续工作。后续配置和订阅扩展继续保持纯内存解析、无隐式网络/文件读取和 secret 不泄露边界。
