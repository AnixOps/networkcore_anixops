...
- Linux artifact readiness/release gate 已纳入 foreground stop/release 合同检查、artifact manifest 输出合同和 license/NOTICE confirmation source contract；当前 pending marker 未确认前不加入 `package-linux`，也不发布 Linux artifact。
