...
- `engine-native::NativeProxyEngineService`，提供原生 engine descriptor、listener/node/route 结构化图校验、service-owned runtime state、`start`/`reload`/`status`/`events`/`stop` 生命周期合同。
...
- `engine-native` 已补充 `NativeHttpMitmPluginHook`、`plan_socks5_connect_http_mitm` 和 caller-provided plain HTTP rewrite 源码合同；当注入的 MITM plugin 对 SOCKS5 CONNECT target 返回 `Reject` 时，accept loop 会在 route/outbound selection 前写 SOCKS5 general failure response 并跳过 outbound，并通过 `engine.native.runtime.http_mitm_connect_browser_proof_observed` 输出由 CONNECT endpoint 和本地 socks5 proxy URL 派生的默认 browser proof token。`NativePlainHttpMessage`/`NativePlainHttpRewriteReport` 可把 plugin outcome 应用到合成明文 HTTP 输入。该能力不解密 TLS，不把 redirect/header/body/script rewrite 应用到真实 live traffic。
- `engine-native` 已补充 service-owned runtime state 与 foreground lifecycle handoff 源码合同；有效配置能生成 native runtime assembly plan 时记录 `engine.native.start.runtime_assembly_ready`，`NativeProxyEngineService::start` 随后启动 loopback TCP accept loop、保存当前进程内 runtime handle 和最后成功配置，并返回 `engine.native.start.running`、`engine.native.runtime.foreground_handoff_ready` 与 `engine.native.runtime.accept_loop_ready`；`reload` 会释放当前 handle、启动 replacement configuration 并输出 `engine.native.reload.running`，若 replacement 启动失败则恢复保存的配置并返回 `engine.native.reload.failed`，恢复也失败时返回 `engine.native.reload.rollback_failed`；`status`/`events`/`stop` 可观察和释放该 runtime。该能力不定义 Linux CLI 跨进程 reload 命令。
...
| `engine.native.runtime.http_mitm_connect_plan_not_applied` | Warning | live HTTP/TLS 数据面尚未应用 redirect/header/body/script MITM plan |
