...
- `engine-native` 已补充从有效配置图生成首个 native runtime assembly plan 的源码合同，选择 loopback TCP listener 与 SOCKS outbound handler，并覆盖绑定失败和 lifecycle handoff 失败的释放报告；该实现尚未接入 `NativeProxyEngineService::start`，也没有 TCP accept loop、proxy protocol handler 或 route/outbound 数据面。
...
- adapter 合同测试覆盖 descriptor、结构化配置拒绝、runtime handle 源码合同、runtime assembly plan、loopback TCP listener 绑定/释放、启动失败、running 状态、status、events 和 secret 不泄露。
