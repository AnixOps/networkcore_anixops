...
- `config-core::CoreSubscriptionService`，提供纯内存 `inline:` subscription source 解析，把最小 subscription TOML `nodes`/`routes` 子集归一化为 `NodeCatalog`；subscription catalog runtime orchestration design 已定义 catalog gate 和重复 id 拒绝边界，当前 runtime start 还不消费 subscription catalog。
