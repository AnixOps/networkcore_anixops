...
- `engine-native` 已补充首个 loopback TCP accept loop 与受控关闭源码合同，覆盖 accepted connection 计数、runtime release 停止报告和 ready/stopped 诊断；该实现尚未接入 `NativeProxyEngineService::start`，也没有 proxy protocol handler 或 route/outbound 数据面。
...
- adapter 合同测试覆盖 descriptor、结构化配置拒绝、runtime handle 源码合同、runtime assembly plan、loopback TCP listener 绑定/释放、loopback TCP accept loop 受控关闭、启动失败、running 状态、status、events 和 secret 不泄露。
