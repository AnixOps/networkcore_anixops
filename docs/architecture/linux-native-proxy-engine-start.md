...
- `engine-native::NativeProxyEngineService`，提供原生 engine descriptor、listener/node/route 结构化图校验、启动不可用、stopped status 和空 events 合同，但没有真实运行句柄。
...
- adapter 合同测试覆盖 descriptor、结构化配置拒绝、启动失败、running 状态、status、events 和 secret 不泄露。
...
- 补充首个 native runtime handle 的最小源码合同，并继续不接入 `networkcore-linux start`。
