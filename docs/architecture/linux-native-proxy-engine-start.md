...
- `engine-native` 已补充 service start readiness gate 诊断合同；有效配置能生成 native runtime assembly plan 时记录 `engine.native.start.runtime_assembly_ready`，随后用 `engine.native.start.service_runtime_owner_missing` 明确阻断 service `start()` 返回 `Running`，直到 service-owned runtime state 与 foreground lifecycle handoff 完成。
...
| `engine.native.start.service_runtime_owner_missing` | Error | service 尚未拥有可跨 start/status/events/stop 生命周期管理的 runtime state |
...
- 补充 `engine-native` service-owned runtime state 与前台 lifecycle handoff 源码合同，继续保持未满足门槛前不接入 `networkcore-linux start` binary。
