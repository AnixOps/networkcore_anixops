...
- `config-core::CoreSubscriptionService`，提供纯内存 `inline:` subscription source 解析，把最小 subscription TOML `nodes`/`routes` 子集归一化为 `NodeCatalog`；当前 runtime start 还不消费 subscription catalog。
