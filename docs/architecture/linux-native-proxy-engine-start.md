...
- Linux artifact readiness/release gate 已纳入 foreground stop/release 合同检查、artifact manifest 输出合同、license/NOTICE confirmation source contract、release placeholder license/NOTICE pending 状态 summary、release CI success source contract 和 package runner/toolchain/target contract；当前 pending marker 未确认前不加入 `package-linux`，也不发布 Linux artifact。
- 下一步补充 `package-linux` archive staging、文件来源和顶层目录组装合同，仍不生成 artifact。
