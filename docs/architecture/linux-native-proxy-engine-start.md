...
该状态是正确的安全边界。`networkcore-linux` binary 的前台 lifecycle host 接线未完成前，不得把
...
当前 adapter 已实现 service-owned 运行句柄和生命周期诊断合同，但仍不能被接入
`networkcore-linux start`，除非二进制入口同时接入前台 lifecycle host。
