...
- Linux artifact readiness/release gate 已纳入 foreground stop/release 合同检查、artifact manifest 输出合同、license/NOTICE confirmation source contract 和 release placeholder license/NOTICE pending 状态 summary；当前 pending marker 未确认前不加入 `package-linux`，也不发布 Linux artifact。
- 下一步补充真实 `package-linux` 前同 commit CI 成功结果读取合同，仍不生成 artifact。
