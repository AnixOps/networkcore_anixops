...
- `networkcore-linux` binary 已将 `prepare-config` 接入 `RuntimeOrchestrator`，并将 `start` 接入 `NativeProxyEngineService` 与 current-process foreground lifecycle host。
...
- `CurrentProcessForegroundLifecycleHost` 稳定持有当前进程的 running handoff，并可注入 interruption source；默认 parking source 继续阻塞持有当前进程，真实 OS signal source 属于后续增量。
...
- 在 foreground lifecycle 退出路径、license/NOTICE 和安装/回滚门禁完成前，不加入 `package-linux`，也不发布 Linux artifact。
