...
- 本文档保持在 README、ROADMAP、Release Strategy、Linux artifact 设计、Linux package
  manifest 设计、Linux CLI artifact 安装/回滚设计和 CI policy 中可发现。
...
- 下一步可以在 release placeholder 中补充 `package-linux` job 的 runner/toolchain/target
  triple 合同，仍不生成 artifact。
