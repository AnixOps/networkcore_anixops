...
该 active gate 证明同 commit CI 已通过，但不证明当前 release 已经可以发布 artifact。
...
- 在 artifact 仍被 license/NOTICE pending marker 和后续 artifact gates 阻止期间，继续输出 source contract 和 active CI 字段。
