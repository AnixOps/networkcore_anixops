...
- 本文档保持在 README、ROADMAP、Release Strategy、Linux artifact 设计、Linux package
  ...
  Linux package runner/toolchain/target contract、Linux package archive staging contract 和 CI policy
  中可发现。
- `.github/workflows/ci.yml` governance 检查本文档存在和标题。
- `.github/workflows/release.yml` 的 `release-ci-gate` 检查本文档存在和标题，并输出
  `release-ci-success-source-contract=present`。
- release summary 输出 release CI source contract 与 required fields。
- 不生成 artifact、不定义 `package-linux`、不上传 release asset、不在本机执行测试、
  构建、打包或发布。

## 后续工作

- 在 artifact 仍被 license/NOTICE pending marker 阻止期间，继续只输出 source contract。
- Linux package runner/toolchain/target contract、Linux package archive staging contract 和 Linux
  package checksum manifest contract 已在 release placeholder 中输出；下一步可以补充
  `package-linux` publish/upload boundary、workflow artifact retention 和 release asset 阻断合同，
  仍不生成 artifact。
