...
- 本文档保持在 README、ROADMAP、Release Strategy、Linux artifact 设计、Linux package
  ...
  Linux package runner/toolchain/target contract、Linux artifact
...
- Linux package checksum manifest contract、publish/upload boundary contract、signing/attestation
  policy binding contract 和 release notes/rollback policy binding contract 已定义；下一步可以补充
  Linux package publish eligibility aggregate contract，仍不生成 artifact。
