...
v0.1.2-alpha.1 的六个 source-only persistent subscription catalog 存储切片均已完成并通过 GitHub Actions 全量 CI：`CommandSubscriptionCatalogStore::add_source`、`list_sources`、`remove_source`、`select_source`、`update_source` 和 `rollback_catalog` 固定显式本地 JSON catalog、schema version 1、写前 rollback snapshot、source-not-found/duplicate 拒绝以及 location 脱敏 report。当前 CLI `subscription update` 在替换 location 前另行执行显式候选 fetch 与 `CoreSubscriptionService` parse/normalize 验证，失败保留旧 catalog；该操作不建立后台刷新，也不接入 `start`、节点选择或 managed lifecycle。边界见 [Persistent Subscription Catalog Source Contract](docs/architecture/subscription-catalog-persistence-source-contract.md)。
