...
- 当前 main 源码状态：`v0.1.0-alpha.14` 发布边界之上已经进入 `v0.1.0-alpha.15` source 增量第一步：`engine-native` 保留 explicit HTTP proxy live plain HTTP data plane，并新增 `NativeTlsMitmFoundationReport`、`plan_explicit_http_connect_tls_mitm_foundation`、`write_http_connect_established_response`、`ENGINE_NATIVE_RUNTIME_HTTP_PROXY_TLS_FOUNDATION_READY_CODE` 和 `ENGINE_NATIVE_RUNTIME_HTTP_PROXY_TLS_CONNECT_TUNNEL_ESTABLISHED_CODE`；`ListenerKind::Http` 的 `CONNECT host:443` path 现在可经既有 SOCKS outbound primitive 建立标准 `HTTP/1.1 200 Connection Established` tunnel 并进行有限双向 relay，作为后续 TLS MITM foundation 的第一步。`MITM_HTTP_TLS_DATA_PLANE_GATE` 仍为 `plain-http-live-data-plane-active/tls-decryption-blocked`，`http_rewrite` report 中 `live_traffic_ready=true`、`tls_decryption_ready=false`；后续 main 源码新增能力仍需要后续新 tag release 才会进入下载包。
- 当前未启用：完整 live MITM、browser hijack、浏览器/系统代理配置 mutation、CA 安装/信任 mutation、profile trust state mutation、HTTPS 解密、TLS termination、HTTPS request/response rewrite、daemon/service、TUN/DNS/firewall mutation。当前 main 只允许写 operator-provided NetworkCore certificate/private-key artifact、dedicated profile CA trust artifact、PAC artifact、可选 Chromium/Chrome managed proxy policy artifact 和 rollback snapshot，并允许显式 caller-provided plain HTTP preview、显式 HTTP proxy listener 的 `http://` live path 应用 rewrite outcome，以及 explicit HTTP proxy `CONNECT` 建立 pass-through tunnel；不安装或信任 CA，不安装系统 PAC 或浏览器 policy，不解析 CONNECT 后 HTTPS 流量。native SOCKS5 hook 仍只能在显式 SOCKS5 CONNECT 层应用插件 `Reject` 为 CONNECT failure，不解密 HTTPS。
