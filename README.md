...
iOS Network Extension design、iOS platform adapter source contract 和首个纯 Rust `platform-ios` 映射骨架已补充；Swift/Xcode project、Network Extension target、签名、TestFlight/App Store 上传和 iOS release asset 均未启用，后续 Apple SDK bridge 必须先补充设计并通过 GitHub Actions 验证。
