...
`mutation_ready=false`，`mitm certificate apply --confirm --cert-file <path> --key-file <path> --snapshot <path>` / `mitm certificate rollback --snapshot <path>` 可写入或删除 NetworkCore-owned certificate/private-key artifact 和 rollback snapshot，并输出 `certificate_lifecycle` JSON report；`browser_plan` 固定浏览器捕获计划、默认显式代理目标
`127.0.0.1:7890`、blocked operations 和 `mutation_ready=false`。`MITM_BROWSER_CAPTURE_GATE`
当前为 pac-policy-profile-prefs-active/system-mutation-blocked，并明确报告 browser hijack 为 `deferred`。证书入口只写调用方指定的 NetworkCore artifact 和 snapshot，不安装或信任 CA，
...
不会安装或信任 CA，不会修改 system trust store，不会解密 HTTPS，不会修改浏览器/系统代理、system PAC、TUN/DNS/firewall；`engine-native`
...
`MITM_CERTIFICATE_LIFECYCLE_GATE` 当前为 artifact-lifecycle-active/trust-mutation-blocked，后续再从 artifact lifecycle 扩展到 CA 安装、信任检测、撤销和 trust-store 回滚；
