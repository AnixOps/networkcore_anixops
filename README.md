...
- 当前 Windows 客户端待发布切片是 `v0.2.0-alpha.16`：`apps/windows-gui`
  ...
  显式切换并读回确认。GUI 也可使用可编辑的 HTTPS `Delay URL` 对当前选择节点做一次
