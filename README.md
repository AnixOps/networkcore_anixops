...
v0.1.2-alpha.2 已完成两个 source-only managed foreground status record 切片：`CommandManagedForegroundSessionStore::read_status` 从显式 schema version 1 JSON record 读取 session id、engine id 和 recorded state；`write_status` 将调用方显式提供的初始 record 非覆盖写入同一 schema。两者固定 `liveness_verified=false`，不验证 PID、端口、socket 或进程存活。它们不接入 CLI `status`、start、events、logs、reload 或 rollback。边界见 [Managed Foreground Session Status Source Contract](docs/architecture/managed-foreground-session-status-source-contract.md)。
