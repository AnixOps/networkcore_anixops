...
- `v0.1.0`：Linux-only explicit HTTPS rewrite preview。alpha.14 已发布 plain HTTP live data plane，alpha.15 已发布 TLS MITM readiness，alpha.16 已发布 controlled TLS termination foundation，alpha.17 已发布 HTTPS request rewrite preview；后续 alpha.18 到 alpha.20 依次推进 HTTPS response rewrite、live browser proof hardening 和 release hardening；不包含 Windows artifact、JavaScript script dispatch、system trust store mutation、system proxy mutation、daemon/service、TUN、DNS 或 firewall mutation。
...
`v0.1.0-alpha.17`，由 GitHub Actions 构建并发布 `networkcore-linux` Linux tarball、sha256、manifest
