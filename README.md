...
补充说明：`networkcore-linux start` binary 已接入 `NativeProxyEngineService` 与前台 lifecycle host；有效 listener/node 配置可让二进制入口在当前进程内启动 loopback TCP accept loop runtime 并进入前台持有路径。`stop` 与后台 `status` 继续保持无 daemon/control socket 边界，Linux artifact 发布仍受 foreground lifecycle 退出路径、安装、license/NOTICE 和回滚门禁阻止。
...
- [apps/linux-cli](apps/linux-cli)：`networkcore-linux` CLI 入口的首批命令解析、配置读取边界、只读平台探测接线、`prepare-config` 运行层接线、`start` 原生 engine 前台接线、前台 lifecycle host 源码合同和诊断输出。
