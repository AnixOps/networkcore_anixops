...
- 订阅和客户端兼容：在 `run-url <ss://url>` foreground 闭环、当前 Trojan/VLESS/VMess URL parser gates、Clash YAML、sing-box JSON、Surge proxy line 和 Loon proxy line catalog import gates 上继续补 Quantumult X 高频子集、VLESS/Trojan/VMess/Clash/sing-box JSON/Surge/Loon runnable path、节点选择、持久订阅和 managed lifecycle。
...
`sing_box_run` 机器字段。Clash YAML、sing-box JSON、Surge proxy line 和 Loon proxy line catalog import parser gates 已进入 current main；Clash/sing-box JSON/Surge/Loon run/render、daemon/control socket、持久订阅、节点选择、Quantumult X 等文档格式、VLESS/Trojan/VMess runnable path、status/events/logs、reload、TUN/DNS mutation 和 MITM 真实流量处理仍是后续工作。
