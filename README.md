...
- MITM 数据面和证书生命周期：补齐 CA 生成/安装/信任/撤销/回滚，以及把 `mitm-policy` `HttpMitmOutcome` rewrite plan 接入 HTTP/TLS 数据面；当前只输出 policy-only 状态、证书/浏览器计划、rich mutation plan，并在 native SOCKS5 CONNECT 前置点应用插件 `Reject` 为 CONNECT failure。
...
`v0.1.0-alpha.9`，由 GitHub Actions 构建并发布 `networkcore-linux` Linux tarball、sha256、manifest
...
命令面和 `browser_capture` 机器字段；当前 main 源码进一步加入 `traffic-proof`、`session-plan`/`launch --confirm` 的 `proof_target_url`/`networkcore_proof_token`/`traffic_proof_command` 绑定、`--proxy-scheme socks5` native plugin proxy mode，以及 `apply --confirm --pac-file <path> [--policy-file <path>] --snapshot <path>` 的 NetworkCore PAC/browser policy artifact 写入/回滚路径。`MITM_BROWSER_CAPTURE_GATE` 现在是
pac-policy-artifact-active/system-mutation-blocked，仍不执行真实系统代理或浏览器配置 mutation。
...
当前为 pac-policy-artifact-active/system-mutation-blocked，并明确报告 browser hijack 为 `deferred`。该入口不生成或安装 CA，
...
`apply --confirm --pac-file <path> [--policy-file <path>] --snapshot <path>` 在 current main 中写入 operator-provided NetworkCore PAC 文件、可选 Chromium/Chrome managed proxy policy artifact 和 rollback snapshot，PAC/policy 内容只指向计划本地代理 `127.0.0.1:7890`，不会安装系统 PAC 或浏览器 policy；缺少 `--pac-file` 或 `--snapshot` 时返回 config missing；`rollback --snapshot <path>` 读取 NetworkCore snapshot 并删除对应 PAC/policy artifact，
...
上述 `launch --confirm`、`verify --confirm`、`traffic-proof --confirm`、proof target URL 绑定、PAC/browser policy artifact apply/rollback 和后续 browser-capture blocked report 已纳入当前 Linux CLI 源码边界；它们只启动 dedicated browser profile、探测本地代理端点、把 proof token 传给目标 URL、检查同一 proof 会话的 proof log token、写入可回滚 PAC/policy artifact 或返回 blocked report，不代表已启用 live MITM。
...
当前仓库源码已有用户可见的 MITM 状态、诊断、证书计划、浏览器捕获计划、PAC/browser policy artifact apply/rollback 和 browser-capture blocked report 入口，但没有用户可启用的 live MITM 功能：
...
不会生成或安装 CA，不会解密 HTTPS，不会修改浏览器/系统代理、system PAC、TUN/DNS/firewall；`engine-native`
...
`MITM_BROWSER_CAPTURE_GATE` 从 pac-policy-artifact-active/system-mutation-blocked 扩展到显式授权的浏览器/系统代理配置、验证和回滚。
