...
当前仓库处于 P2 初始内核骨架阶段，已建立协作规范、规划治理入口、架构规格、运行层编排设计、发布策略、iOS 平台风险评估、Rust 首选栈决策、最小 `control-domain` crate、最小 `control-runtime` crate、MITM gate 初始门禁用例、平台 MITM 不可用拒绝路径、证书状态拒绝矩阵、manifest 诊断拒绝路径、manifest 非错误诊断聚合路径、远程脚本执行拒绝路径、远程脚本未知状态拒绝路径、权限拒绝审计边界、审计事件聚合边界、平台能力状态类型和 Rust 依赖安全扫描 CI。后续实现必须先补齐对应规格或设计说明，并通过 CI/CD 验证。
