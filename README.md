...
- 当前已发布的 Windows 客户端切片是 `v0.2.0-alpha.1`：`apps/windows-gui`
  ...
  lifecycle 的 apply/rollback；JavaScript dispatch 与 Windows 订阅 runnable
  path 仍未激活。发布合同见
...
Linux/Windows 发布路径已打通：Linux 首个真实发布路径从 `v0.1.0-alpha.2` 开始，Windows managed-client MSI 从 `v0.2.0-alpha.1` 开始；当前最新 GitHub Release 是
`v0.2.0-alpha.1` prerelease，最新 stable 是 `v0.1.0`。`v0.2.0-alpha.1` 由 GitHub Actions 构建并发布
`networkcore-linux` Linux tarball、sha256、manifest、manifest sha256，以及 `networkcore-windows` managed-client MSI、sha256、manifest、manifest sha256，历史 `v0.1.1-alpha.2` manual-extract zip 仅作审计记录，release workflow 同时完成
...
最新已发布 prerelease 是 `v0.2.0-alpha.1`，最新 stable 是 `v0.1.0`；
Linux 仍是手动解压和 foreground/CLI 运行模型，不安装 daemon/service，不修改 TUN/DNS/firewall/certificate trust store；Windows 已通过 MSI 提供 managed GUI/service、installer 和可回滚的系统代理/信任库生命周期。
