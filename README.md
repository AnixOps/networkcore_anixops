...
v0.1.2-alpha.1 的五个 source-only persistent subscription catalog 切片均已完成并通过 GitHub Actions 全量 CI：`CommandSubscriptionCatalogStore::add_source`、`list_sources`、`remove_source`、`select_source` 和 `update_source` 固定显式本地 JSON catalog、schema version 1、写前 rollback snapshot、source-not-found/duplicate 拒绝以及 location 脱敏 report；`update` 只替换已存在 source 的 location 并保持 source id。该阶段不执行默认路径扫描、远程或文件订阅读取，也不接入 `start`、节点选择或 managed lifecycle。边界见 [Persistent Subscription Catalog Source Contract](docs/architecture/subscription-catalog-persistence-source-contract.md)。
