...
- 订阅和客户端兼容：在 `run-url <ss://url>` foreground 闭环、当前 Trojan/VLESS/VMess URL parser gates 和 Clash YAML catalog import gate 上继续补 sing-box JSON、Surge/Loon/Quantumult X 高频子集、VLESS/Trojan/VMess/Clash runnable path、节点选择、持久订阅和 managed lifecycle。
...
- `v0.1.1`：正式引入 Windows 版本，并把订阅兼容作为主线。Windows 首期目标是 CLI artifact 和 release path，不默认包含 Windows service、driver、installer 或系统代理 mutation；订阅侧已推进 Trojan/VLESS/VMess 和 Clash YAML parser gates，后续推进 sing-box JSON、VLESS/Trojan/VMess/Clash runnable path、节点选择和 cross-platform run plan。
...
`sing_box_run` 机器字段。Clash YAML catalog import parser gate 已进入 current main；Clash run/render、daemon/control socket、持久订阅、节点选择、sing-box JSON 等文档格式、VLESS/Trojan/VMess runnable path、status/events/logs、reload、TUN/DNS mutation 和 MITM 真实流量处理仍是后续工作。
