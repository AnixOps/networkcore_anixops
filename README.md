...
P3 subscription catalog runtime orchestration design 已补充，定义 `CoreSubscriptionService` 产出的 `NodeCatalog` 进入 `RuntimeConfigRequest.nodes` 前的运行层 gate、与 `ConfigSnapshot.nodes` 的重复 id 拒绝、`NodeCatalog.rules` deferred 语义和 no remote/file subscription、no system DNS/TUN mutation、no daemon/control socket 边界。源码仍未把 subscription catalog 接入 `RuntimeOrchestrator` 或 `networkcore-linux start`。
...
- [crates/control-runtime](crates/control-runtime)：组合领域端口的首批纯运行层编排用例；P3 subscription catalog runtime orchestration design 已定义 `NodeCatalog` 进入 `RuntimeConfigRequest.nodes` 前的运行层 gate，源码仍未消费 subscription catalog。
