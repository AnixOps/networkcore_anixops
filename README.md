...
补充说明：`engine-native` 已新增 service start readiness gate，能确认有效配置已具备 native runtime assembly plan，但 `NativeProxyEngineService::start` 仍因 service-owned runtime state 与 foreground lifecycle handoff 未接线而返回稳定 unavailable 诊断；`networkcore-linux start` 继续保持未接线。
