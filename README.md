...
iOS Network Extension design 和 iOS platform adapter source contract 已补充，当前仍为 design-only；Swift/Xcode project、Network Extension target、签名、TestFlight/App Store 上传和 iOS release asset 均未启用，后续 `platform-ios` 源码必须先按 source contract 落地并通过 GitHub Actions 验证。
