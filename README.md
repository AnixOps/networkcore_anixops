...
Linux CLI 二进制发布路径已进入 release workflow：`package-linux` 会在 GitHub Actions 中构建
`networkcore-linux`、生成 tarball、sha256、manifest 和 manifest sha256，`attest-linux` 会生成 GitHub
artifact attestation，tag 触发的 `publish-github-release` 会在全部门禁通过后上传 GitHub Release assets。
当前仍等待 `docs/manual-intervention.md` 中的 Linux artifact license/NOTICE 人工确认；marker 为 pending 时
`linux-artifact-readiness` 会失败，因此不会构建、上传或发布二进制。

说明：下方历史清单中保留了 placeholder 阶段的字段名称；当前可执行状态以上面三段为准：
`package-linux`、`attest-linux`、`publish-eligibility-gate` 和 `publish-github-release` 已定义，
但 license/NOTICE marker 为 pending 时会停在 `linux-artifact-readiness`，不会生成 release asset。
