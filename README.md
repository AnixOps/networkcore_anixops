...
- 订阅和客户端兼容：在 `run-url <ss://url>` foreground 闭环和当前 Trojan/VLESS URL parser gates 上继续补 VMess、Clash YAML、sing-box JSON、Surge/Loon/Quantumult X 高频子集、VLESS/Trojan runnable path、节点选择、持久订阅和 managed lifecycle。
...
- `v0.1.1`：正式引入 Windows 版本，并把订阅兼容作为主线。Windows 首期目标是 CLI artifact 和 release path，不默认包含 Windows service、driver、installer 或系统代理 mutation；订阅侧推进 Trojan/VLESS parser gates、VMess、Clash YAML、sing-box JSON、VLESS/Trojan runnable path、节点选择和 cross-platform run plan。
...
`sing_box_run` 机器字段。daemon/control socket、持久订阅、节点选择、VMess/Clash 等格式、
VLESS/Trojan runnable path、status/events/logs、reload、TUN/DNS mutation 和 MITM 真实流量处理仍是后续工作。
