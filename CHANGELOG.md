...
- 在 `engine-native` 中新增 service start readiness gate 诊断合同；`assess_native_proxy_engine_start_readiness` 会先复用结构化配置校验，再确认有效配置可生成 native runtime assembly plan 并记录 `engine.native.start.runtime_assembly_ready`，随后用 `engine.native.start.service_runtime_owner_missing` 明确阻断 `NativeProxyEngineService::start` 返回 `Running`，因为 service-owned runtime state 与 foreground lifecycle handoff 尚未接线；`start` 继续返回稳定 `engine.native.start.runtime_unavailable`，`networkcore-linux start` 继续保持未接线。
