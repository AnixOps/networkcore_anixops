...
The crate is intentionally library-only. It defines shared value types and service traits for configuration, listener configuration, subscription parsing, policy routing, platform capability status, proxy engine adapters, DNS policy, MITM plugins, and the control API. Platform SDKs, proxy engine process management, UI code, and transport-specific control APIs must live in later adapter crates.
...
The `maintenance` module provides versioned serde JSON and RFC3339 validation using pinned workspace-lock dependencies. Its strict event and batch entrypoints validate authenticated identity supplied by a host, preserve bad-event isolation, and expose failure/recovery evidence thresholds. This is an explicit serialization boundary, without transport, durable queue, incident persistence or runtime liveness claims. See [maintenance handover](../../docs/MAINTENANCE_P0.md).
