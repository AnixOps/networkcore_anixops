...
The crate is intentionally library-only and dependency-free. It defines shared value types and service traits for configuration, subscription parsing, policy routing, DNS policy, MITM plugins, and the control API. Platform SDKs, proxy engine process management, UI code, and transport-specific control APIs must live in later adapter crates.
