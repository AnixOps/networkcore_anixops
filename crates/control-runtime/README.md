...
The crate depends only on `control-domain`. It composes configuration, platform capability, and proxy engine ports to prepare configuration, start, reload, stop, inspect status, and read events without depending on platform SDKs, external proxy binaries, UI frameworks, network transports, or local filesystem configuration.
