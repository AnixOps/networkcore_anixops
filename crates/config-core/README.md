...
The crate also implements `SubscriptionService` as `CoreSubscriptionService`. It accepts explicit `inline:` subscription sources, parses the same minimal TOML `nodes` and `routes` subset, single `ss://` URLs, plaintext `ss://` link lists, and base64 encoded `ss://` link lists into `SubscriptionDocument`, and normalizes them into `NodeCatalog`. Remote subscription fetching, file loading, authentication, DNS, plugin, duplicate-id, and listener/node graph validation remain out of scope for this crate; secrets are only carried as node metadata for adapter translation and must not be echoed in diagnostics.
